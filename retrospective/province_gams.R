...
get_partial_preds <- function(gamFit, newdata, terms, seWithMean) {
  ...
  tmp_preds <- predict(gamFit, newdata = newdata, se.fit = T,
          type = predType, terms = terms) %>% lapply(rowSums) %>% as.data.frame()
...
correlation_gravity_gams <- function(data){
  ...
  #Formulae
  tmax_pop_prec_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                +s(tmax, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                                +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")
                                , 
                                data = data)
  tmax_pop_spi_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                               +s(tmax, bs="tp") + s(POP_1_2, bs="tp") +s(spi, bs="tp") 
                               +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")
                               , 
                               data = data)
  tmax_pop_prec_icen_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                     +s(tmax, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                                     +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")+
                                       s(icen, bs="tp")
                                     , 
                                     data = data)
  tmax_prec_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                            +s(tmax, bs="tp") +s(prec, bs="tp") 
                            +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")
                            , 
                            data = data)
  tmax_prec_icen_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                 +s(tmax, bs="tp") +s(prec, bs="tp") 
                                 +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")+
                                   s(icen, bs="tp")
                                 , 
  ...
  tmin_pop_prec_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                +s(tmin, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                                +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")
                                , 
                                data = data)
  tmin_pop_spi_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                               +s(tmin, bs="tp") + s(POP_1_2, bs="tp") +s(spi, bs="tp") 
                               +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")
                               , 
                               data = data)
  tmin_pop_prec_icen_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                     +s(tmin, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                                     +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")+
                                       s(icen, bs="tp")
                                     , 
                                     data = data)
  tmin_prec_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                            +s(tmin, bs="tp") +s(prec, bs="tp") 
                            +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")
                            , 
                            data = data)
  tmin_prec_icen_year_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                 +s(tmin, bs="tp") +s(prec, bs="tp") 
                                 +s(YEAR_FACTOR, bs="re")+s(PROVINCE_FACTOR, bs="re")+
                                   s(icen, bs="tp")
                                 , 
  ...
  tmax_pop_prec_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                           +s(tmax, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                           +s(PROVINCE_FACTOR, bs="re")
                           , 
                           data = data)
  tmax_pop_spi_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                          +s(tmax, bs="tp") + s(POP_1_2, bs="tp") +s(spi, bs="tp") 
                          +s(PROVINCE_FACTOR, bs="re")
                          , 
                          data = data)
  tmax_pop_prec_icen_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                +s(tmax, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                                +s(PROVINCE_FACTOR, bs="re")+
                                  s(icen, bs="tp")
                                , 
                                data = data)
  tmax_prec_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                       +s(tmax, bs="tp") +s(prec, bs="tp") 
                       +s(PROVINCE_FACTOR, bs="re")
                       , 
                       data = data)
  tmax_prec_icen_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                            +s(tmax, bs="tp") +s(prec, bs="tp") 
                            +s(PROVINCE_FACTOR, bs="re")+
                              s(icen, bs="tp")
                            , 
  ...
  tmin_pop_prec_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                           +s(tmin, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                           +s(PROVINCE_FACTOR, bs="re")
                           , 
                           data = data)
  tmin_pop_spi_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                          +s(tmin, bs="tp") + s(POP_1_2, bs="tp") +s(spi, bs="tp") 
                          +s(PROVINCE_FACTOR, bs="re")
                          , 
                          data = data)
  tmin_pop_prec_icen_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                                +s(tmin, bs="tp") + s(POP_1_2, bs="tp") +s(prec, bs="tp") 
                                +s(PROVINCE_FACTOR, bs="re")+
                                  s(icen, bs="tp")
                                , 
                                data = data)
  tmin_prec_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                       +s(tmin, bs="tp") +s(prec, bs="tp") 
                       +s(PROVINCE_FACTOR, bs="re")
                       , 
                       data = data)
  tmin_prec_icen_fit <- gam(corr ~ dist_km:PROVINCE_FACTOR 
                            +s(tmin, bs="tp") +s(prec, bs="tp") 
                            +s(PROVINCE_FACTOR, bs="re")+
                              s(icen, bs="tp")
                            , 
                            data = data)
  model_fits <- list(tmax_pop_prec_year_fit, tmax_pop_spi_year_fit, tmax_pop_prec_icen_year_fit,
  ...
  model_names <- c("tmax_pop_prec_year_fit", "tmax_pop_spi_year_fit", "tmax_pop_prec_icen_year_fit",
                   ...
                   "tmax_prec_fit", "tmax_prec_icen_fit", "tmin_pop_prec_fit", "tmin_pop_spi_fit", "tmin_pop_prec_icen_fit", 
                   "tmin_prec_fit", "tmin_prec_icen_fit")
  aic_vals <- c(tmax_pop_prec_year_fit$aic, tmax_pop_spi_year_fit$aic, tmax_pop_prec_icen_year_fit$aic,
  ...
  bic_vals <- c(BIC(tmax_pop_prec_year_fit), BIC(tmax_pop_spi_year_fit), BIC(tmax_pop_prec_icen_year_fit),
  ...
  aicc_vals <- c(AICc(tmax_pop_prec_year_fit), AICc(tmax_pop_spi_year_fit), AICc(tmax_pop_prec_icen_year_fit),
  ...
  gcv_vals <- c(tmax_pop_prec_year_fit$gcv.ubre, tmax_pop_spi_year_fit$gcv.ubre, tmax_pop_prec_icen_year_fit$gcv.ubre,
  ...
  model_summary <- data.table(MODEL = model_names, AIC = aic_vals, BIC = bic_vals, gcv = gcv_vals,
                              aicc = aicc_vals,
...
corr_tmin_pop_prec_year_gravity_gam_fit.tmin.df <- cbind.data.frame(tmin=corr_tmin_pop_prec_year_gravity_gam_fit.out.tmin$effects$tmin, y=corr_tmin_pop_prec_year_gravity_gam_fit.out.tmin$effects$y, 
...
corr_tmin_pop_prec_year_gravity_gam_fit.prec.df <- cbind.data.frame(prec=corr_tmin_pop_prec_year_gravity_gam_fit.out.prec$effects$prec, y=corr_tmin_pop_prec_year_gravity_gam_fit.out.prec$effects$y, 
...
corr_tmin_pop_prec_year_gravity_gam_fit.pop.df <- cbind.data.frame(POP=corr_tmin_pop_prec_year_gravity_gam_fit.out.pop$effects$POP, y=corr_tmin_pop_prec_year_gravity_gam_fit.out.pop$effects$y, 
...
corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects <- ggplot(data=corr_tmin_pop_prec_year_gravity_gam_fit.fixed.df) + theme_bw() + coord_flip() +
...
dev.off()
corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.year.effects <- ggplot(data=corr_tmin_pop_prec_year_gravity_gam_fit.year.df) + theme_bw() + 
  ...
  geom_hline(aes(yintercept=0))+ xlab("Year") + 
...
corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.tmin.effects <- 
  ggplot(data=corr_tmin_pop_prec_year_gravity_gam_fit.tmin.df) + theme_bw() +
...
corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.prec.effects <- ggplot(data=corr_tmin_pop_prec_year_gravity_gam_fit.prec.df) + theme_bw() + 
  theme(panel.grid = element_blank()) +
  ...
  geom_hline(aes(yintercept=0))+ 
  geom_line(aes(x=prec, y=y), show.legend = F, size=3,
            color= "turquoise") +
...
corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.pop.effects <- ggplot(data=corr_tmin_pop_prec_year_gravity_gam_fit.pop.df) + theme_bw() +
  theme(panel.grid = element_blank()) +
...
corr_tmin_pop_prec_year_gravity_gam_plot <-
  ggarrange(corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects + theme(plot.margin = unit(c(0.8, 0.8, 0.8, 0.8), 
                                                                                             "inches")) + theme(text = element_text(size = 22),
          ...
          corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.year.effects + theme(plot.margin = unit(c(0.8, 0.8, 0.8, 0.8), 
                                                                                                        "inches"))  + theme(text = element_text(size = 22),
                                                                                     axis.text.x = element_text(size=22),
                                                                                     axis.text.y = element_text(size=22),
                                                                                     panel.grid.minor.y = element_blank(),
                                                                                     ...
                                                                                     axis.title=element_text(size=22), 
                                                                                     legend.text=element_text(size=22)+geom_text(size = 22)),
          corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.tmin.effects + theme(plot.margin = unit(c(0.8, 0.8, 0.8, 0.8), 
                                                                                                        "inches"))  + theme(text = element_text(size = 22),
                                                                                     axis.text.x = element_text(size=22),
                                                                                     axis.text.y = element_text(size=22),
                                                                                     panel.grid.minor.y = element_blank(),
                                                                                     ...
                                                                                     axis.title=element_text(size=22), 
                                                                                     legend.text=element_text(size=22)+geom_text(size = 22)),
          corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.prec.effects + theme(plot.margin = unit(c(0.8, 0.8, 0.8, 0.8), 
                                                                                                        "inches"))  +theme(text = element_text(size = 22),
                                                                                     axis.text.x = element_text(size=22),
                                                                                     axis.text.y = element_text(size=22),
                                                                                     panel.grid.minor.y = element_blank(),
                                                                                     ...
                                                                                     axis.title=element_text(size=22), 
                                                                                     legend.text=element_text(size=22)+geom_text(size = 22)),
          corr_tmin_pop_prec_year_gravity_gam_fit.fixed.effects.pop.effects + theme(plot.margin = unit(c(0.8, 0.8, 0.8, 0.8), 
                                                                                                       "inches"))  +theme(text = element_text(size = 22),
                                                                                    axis.text.x = element_text(size=22),
                                                                                    axis.text.y = element_text(size=22),
                                                                                    panel.grid.minor.y = element_blank(),
...
ggsave(corr_tmin_pop_prec_year_gravity_gam_plot, 
       ...
       h = 24, w = 24) #Plot for paper of effects
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.tmax.df <- cbind.data.frame(tmax=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.tmax$effects$tmax, y=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.tmax$effects$y, 
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.prec.df <- cbind.data.frame(prec=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.prec$effects$prec, y=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.prec$effects$y, 
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.icen.df <- cbind.data.frame(icen=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.icen$effects$icen, y=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.icen$effects$y, 
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.pop.df <- cbind.data.frame(POP=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.pop$effects$POP, y=corr_tmax_pop_prec_icen_year_gravity_gam_fit.out.pop$effects$y, 
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.fixed.effects <- ggplot(data=corr_tmax_pop_prec_icen_year_gravity_gam_fit.fixed.df) + theme_bw() + coord_flip() +
  theme(panel.grid = element_blank(), axis.title.y = element_blank()) +
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.fixed.effects.year.effects <- ggplot(data=corr_tmax_pop_prec_icen_year_gravity_gam_fit.year.df) + theme_bw() + 
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.fixed.effects.tmax.effects <- ggplot(data=corr_tmax_pop_prec_icen_year_gravity_gam_fit.tmax.df) + theme_bw() +
  theme(panel.grid = element_blank()) +
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.fixed.effects.prec.effects <- ggplot(data=corr_tmax_pop_prec_icen_year_gravity_gam_fit.prec.df) + theme_bw() + 
  theme(panel.grid = element_blank()) +
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.fixed.effects.icen.effects <- 
  ggplot(data=corr_tmax_pop_prec_icen_year_gravity_gam_fit.icen.df) + theme_bw() + 
  theme(panel.grid = element_blank()) +
...
corr_tmax_pop_prec_icen_year_gravity_gam_fit.fixed.effects.pop.effects <- ggplot(data=corr_tmax_pop_prec_icen_year_gravity_gam_fit.pop.df) + theme_bw() +
  theme(panel.grid = element_blank()) +
...
corr_tmin_pop_prec_year_gam_fit.tmin.df <- cbind.data.frame(tmin=corr_tmin_pop_prec_year_gam_fit.out.tmin$effects$tmin, y=corr_tmin_pop_prec_year_gam_fit.out.tmin$effects$y, 
...
corr_tmin_pop_prec_year_gam_fit.prec.df <- cbind.data.frame(prec=corr_tmin_pop_prec_year_gam_fit.out.prec$effects$prec, y=corr_tmin_pop_prec_year_gam_fit.out.prec$effects$y, 
...
corr_tmin_pop_prec_year_gam_fit.pop.df <- cbind.data.frame(POP=corr_tmin_pop_prec_year_gam_fit.out.pop$effects$POP, y=corr_tmin_pop_prec_year_gam_fit.out.pop$effects$y, 
...
corr_tmin_pop_prec_year_gam_fit.fixed.effects <- ggplot(data=corr_tmin_pop_prec_year_gam_fit.fixed.df) + theme_bw() + coord_flip() +
  theme(panel.grid = element_blank(), axis.title.y = element_blank()) +
  ylab(bquote(atop("fixed effect of province and geographic distance", "interaction on pairwise correlation coefficient,"~rho)))+
  geom_hline(aes(yintercept=0))+ 
  geom_point(aes(x=PROVINCE_FACTOR, y=y, color=IsSig), show.legend = F, size=3) +
...
corr_tmin_pop_prec_year_gam_fit.fixed.effects.year.effects <- ggplot(data=corr_tmin_pop_prec_year_gam_fit.year.df) + theme_bw() + 
  theme(panel.grid = element_blank(), axis.title.x = element_blank(), axis.text.x = element_text(angle=300, vjust = 0)) +
  ylab(bquote(atop("partial effect of year on", "pairwise correlation coefficient,"~rho)))+
  geom_hline(aes(yintercept=0))+ 
  geom_point(aes(x=YEAR_FACTOR, y=y, color=IsSig), show.legend = F, size=3) +
...
corr_tmin_pop_prec_year_gam_fit.fixed.effects.tmin.effects <- ggplot(data=corr_tmin_pop_prec_year_gam_fit.tmin.df) + theme_bw() +
  theme(panel.grid = element_blank()) +
  ylab(bquote(atop("partial effect of temperature on", "pairwise correlation coefficient,"~rho)))+
  xlab(bquote("mean biweekly temperature ("^0~"C)"))+
  geom_hline(aes(yintercept=0))+ 
...
corr_tmin_pop_prec_year_gam_fit.fixed.effects.prec.effects <- ggplot(data=corr_tmin_pop_prec_year_gam_fit.prec.df) + theme_bw() + 
  theme(panel.grid = element_blank()) +
  ylab(bquote(atop("partial effect of precipitation on", "pairwise correlation coefficient,"~rho)))+
  xlab("mean total annual precipitation (mm)")+
  geom_hline(aes(yintercept=0))+ 
...
corr_tmin_pop_prec_year_gam_fit.fixed.effects.pop.effects <- ggplot(data=corr_tmin_pop_prec_year_gam_fit.pop.df) + theme_bw() +
  theme(panel.grid = element_blank()) +
  ylab(bquote(atop("partial effect of province population on", "pairwise correlation coefficient,"~rho)))+
  xlab("mean population size")+
  geom_hline(aes(yintercept=0))+ 
