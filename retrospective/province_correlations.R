...
monthly_longitude_plot_by_year <- ggplot(ptl_province_inla_df) +
  ...
  scale_color_viridis_c(name = "Log(Cases+1)") +
  ylab("") +
...
province.mean.case.pearsons.df <- annual.case.pearsons.df[, list(
...
annual.case.pearsons.df2[which(YEAR == 2017), list(
  corr = mean(corr),
  DIR = mean(DIR)
), by = "PROVINCE"]
...
p1 <- ggplot(avg.annual.case.pearsons.df2.by.year) +
  geom_boxplot(aes(x = YEAR_FACTOR, y = corr, fill = corr)) +
  theme_bw() +
  scale_fill_viridis()
p2 <- ggplot(avg.annual.case.pearsons.df2.by.year) +
  geom_boxplot(aes(x = YEAR_FACTOR, y = DIR, fill = DIR)) +
  theme_bw() +
  scale_fill_viridis()
...
ptl_province_peru_map_annual.case_corr_betw_dist_and_correlation <-
  ...
  geom_sf(aes(fill = annual.case_corr_betw_dist_and_correlation), alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Longitude") +
  ylab("Latitude") +
...
yearly_pearsons_dt
ggplot(yearly_pearsons_dt, aes(x = YEAR, y = corr)) +
  geom_line()
...
annual_case_distribution_dt <-
  latitude_annual.case.pearsons.df[, list(
...
lat_line_plot_annual_case <- ggplot(annual_case_distribution_dt) +
  ...
  geom_ribbon(aes(x = YEAR, ymin = CI_L, ymax = CI_U),
    alpha = 0.2, fill = "pink"
  ) +
  theme_bw() +
  theme(legend.position = "bottom")
...
annual_cycle_distribution_dt <-
  annual.cycle.pearsons.df[which(!is.na(corr)), list(
    MEDIAN = median(corr, na.rm = TRUE),
...
multi.cycle.pearsons.df <- merge(multi.cycle.pearsons.df,
  unique(subset(ptl_province_inla_df,
    select = c(
      "PROVINCE", "LONG_PROV_IND", "LAT_PROV_IND",
      "REGION"
    )
  )),
  by = "PROVINCE"
...
long_plot_multi_cycle_overlapping <- ggplot(data = avg_multi.cycle.pearsons.df) +
  ...
  scale_fill_viridis_c(name = "Mean correlation") +
...
summary_corrs <- ldply(
  list(median_annual_case_corr, median_annual_cycle_corr, median_multi_cycle_corr),
  function(x) process_correlations_df(x)
)
summary_corrs <- data.table(summary_corrs)
summary_corrs[, time_series := c(
  "Raw Incidence", "Reconstructed Annual Cycle",
...
ptl_province_peru_map_median_annual_case_corr <-
  ggplot(ptl_province_peru_dt, aes(geometry = geometry)) +
  geom_sf(aes(fill = median_annual_case_corr), alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Longitude") +
  ylab("Latitude") +
...
ptl_province_peru_map_median_annual_cycle_corr <-
  ggplot(ptl_province_peru_dt, aes(geometry = geometry)) +
  geom_sf(aes(fill = median_annual_cycle_corr), alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Longitude") +
  ylab("Latitude") +
...
ptl_province_peru_map_median_multi_cycle_corr <-
  ggplot(ptl_province_peru_dt, aes(geometry = geometry)) +
  geom_sf(aes(fill = median_multi_cycle_corr), alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Longitude") +
  ylab("Latitude") +
...
ggplot(tmp) +
  geom_point(aes(x = dist_km, y = full_ts_corr))
...
annual_case_distribution_dt <-
  annual.case.pearsons.df[which(!is.na(corr)), list(
    MEDIAN = median(corr, na.rm = TRUE),
    MAX = quantile(corr, probs = 0.75),
    MIN = quantile(corr, probs = 0.25)
  ),
...
line_plot_multi.cycle <- ggplot(multi.cycle_distribution_dt) +
  geom_line(aes(x = mid_YEAR, y = MEDIAN),
    color = "darkorchid3",
    linewidth = 1.5
  ) +
  geom_ribbon(aes(x = mid_YEAR, ymin = MIN, ymax = MAX),
