...
counter = 0
for i in range(14):
    filename_i = f'/app/data/python/output/historical_no_covars_timegpt_dataframe_{i}.csv'
    if Path(filename_i).exists():
...
counter = 0
for i in range(14):
    filename_i = f'/app/data/python/output/finetuned_no_covars_timegpt_dataframe_{i}.csv'
    if Path(filename_i).exists():
