...
province_first_time_2018 <- head(ptl_province_inla_df[which(YEAR == 2018), ]$TIME,
    1) - 1
...
# JSB: This next block is another loop that needs to be run in parallel as part
# of a workflow run.

# Run model for 2010-2017 inclusive---- Leave-future-out forecasting
log_info("Running model for 2010-2017 inclusive (n=", province_first_time_2018 -
    1, ")")
for (i in 1:(province_first_time_2018 - 1)) {

    filename_i <- file.path(peru.province.inla.data.out.dir, paste0("zi_pois_season_sq_rsi_ns_mod_historical_tmin_roll_2_prec_roll_2_spi_icen_historical_dir.pred",
        i, ".RDS"))
    if (file.exists(filename_i)) {
        ...
        historical_rt_forecast_lag_tmax <- tsModel::Lag(ptl_climate_dt_province$tmax,
            group = ptl_climate_dt_province$PROVINCE, k = 1:maximum_climate_lag)
        historical_rt_forecast_lag_tmax <- historical_rt_forecast_lag_tmax[57:nrow(historical_rt_forecast_lag_tmax),
            ]
        historical_rt_forecast_lag_tmax <- historical_rt_forecast_lag_tmax[1:nrow(historical_rt_forecast_dt),
            1:3]
        ...
        historical_rt_forecast_lag_tmin <- historical_rt_forecast_lag_tmin[1:nrow(historical_rt_forecast_dt),
            1:3]
        ...
        historical_rt_forecast_lag_prec <- historical_rt_forecast_lag_prec[1:nrow(historical_rt_forecast_dt),
            1:3]
        ...
        historical_rt_forecast_lag_tmax_roll_2 <- historical_rt_forecast_lag_tmax_roll_2[1:nrow(historical_rt_forecast_dt),
            1:3]
        ...
        historical_rt_forecast_lag_tmin_roll_2 <- historical_rt_forecast_lag_tmin_roll_2[1:nrow(historical_rt_forecast_dt),
            1:3]
        ...
        historical_rt_forecast_lag_prec_roll_2 <- historical_rt_forecast_lag_prec_roll_2[1:nrow(historical_rt_forecast_dt),
            1:3]
        # SPI-6
        ...
        historical_rt_forecast_lag_spi <- historical_rt_forecast_lag_spi[1:nrow(historical_rt_forecast_dt),
            1:2]
        ...
        historical_rt_forecast_lag_oni <- historical_rt_forecast_lag_oni[1:nrow(historical_rt_forecast_dt),
            1:4]
        ...
        historical_rt_forecast_lag_icen <- historical_rt_forecast_lag_icen[1:nrow(historical_rt_forecast_dt),
            1:4]

        lagknot <- c(1, 2)
        tmax_basis <- crossbasis(historical_rt_forecast_lag_tmax, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$tmax, 2)), arglag = list(fun = "bs",
            knots = lagknot))
        tmin_basis <- crossbasis(historical_rt_forecast_lag_tmin, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$tmin, 2)), arglag = list(fun = "bs",
            knots = lagknot))

        prec_basis <- crossbasis(historical_rt_forecast_lag_prec, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$prec, 2)), arglag = list(fun = "bs",
            knots = lagknot))

        tmax_roll_2_basis <- crossbasis(historical_rt_forecast_lag_tmax_roll_2, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$tmax_roll_2, 2)), arglag = list(fun = "bs",
            knots = lagknot))
        tmin_roll_2_basis <- crossbasis(historical_rt_forecast_lag_tmin_roll_2, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$tmin_roll_2, 2)), arglag = list(fun = "bs",
            knots = lagknot))

        prec_roll_2_basis <- crossbasis(historical_rt_forecast_lag_prec_roll_2, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$prec_roll_2, 2)), arglag = list(fun = "bs",
            knots = lagknot))

        spi_basis <- crossbasis(historical_rt_forecast_lag_spi, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$SPI_6, 2)), arglag = list(fun = "bs",
            knots = 1))

        oni_basis <- crossbasis(historical_rt_forecast_lag_oni, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$ANOM, 2)), arglag = list(fun = "bs",
            knots = 2))
        icen_basis <- crossbasis(historical_rt_forecast_lag_icen, argvar = list(fun = "bs",
            knots = equalknots(ptl_climate_dt_province$E_INDEX, 2)), arglag = list(fun = "bs",
            knots = 2))
        ...
        forecast_formula <- CASES ~ 1 + f(MONTH, replicate = PROV_IND, model = "rw1",
            cyclic = TRUE, constr = TRUE, scale.model = TRUE, hyper = prior.prec) +
            f(YEAR, replicate = PROV_IND, model = "iid") + f(PROV_IND, model = "bym2",
            hyper = prior.prec, scale.model = TRUE, graph = file.path(peru.province.inla.data.in.dir,
                "nbr_piura_tumbes_lambayeque.graph")) + SQ_RSI_DIR_LAG + SEASON +
            ns(MODIFIED_DIFF_WITH_HISTORICAL_DIR_LAG, df = 4) + tmin_roll_2_basis +
            prec_roll_2_basis + icen_basis + spi_basis

        tmp_climate_cv_fit <- run_province_model_func(data = historical_rt_forecast_dt,
        ...
        for (s.idx in 1:s) {
            ...
            y.pred[, s.idx] <- rzipois(num_provinces, lambda = exp(xx.sample[-1]),
                pstr0 = xx.sample[1])
            dir.pred[, s.idx] <- y.pred[, s.idx]/pop_offsets
        }
        log_info("Saving file: ", filename_i)
        saveRDS(dir.pred, file = p01_filename)
    }
}
