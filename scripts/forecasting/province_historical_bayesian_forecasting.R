...
saveRDS(historical_ovr_province_rt_forecast_preds_dt, file = file.path(
  peru.province.inla.data.out.dir,
...
historical_ovr_province_rt_forecast_preds_dt <- readRDS(file = file.path(
  peru.province.inla.data.out.dir,
  paste0("historical_ovr_province_rt_forecast_preds_dt.RDS")
))
...
# Proper Scoring ----
historical_all_pit <- scoringutils::pit_sample(
  historical_all_true_dirs,
  historical_all_dir.pred
)
hist(historical_all_pit)

historical_all_log_scores <- scoringutils::logs_sample(
  historical_all_true_dirs,
  historical_all_dir.pred
)
...
saveRDS(historical_all_crps, file = file.path(
  peru.province.inla.data.out.dir,
  paste0("historical_all_crps.RDS")
))

province_historical_all_log_scores_dt <- data.table(
  LOG_SCORE = historical_all_log_scores,
...
ggplot(province_historical_all_log_scores_dt) +
  geom_histogram(aes(x = PIT_VAL)) +
  facet_wrap(PROVINCE ~ ., scales = "free_y") +
  theme_bw()

ggplot(province_historical_all_log_scores_dt) +
  geom_histogram(aes(x = LOG_SCORE)) +
  facet_wrap(PROVINCE ~ ., scales = "free_y") +
  theme_bw()
...
median_province_log_score_climate_forecasting <-
  province_historical_all_log_scores_dt[, list(
    MEDIAN_LOG_SCORE = median(LOG_SCORE, na.rm = TRUE),
...
historical_dir_rt_forecast_ptl_pred_dt[, list(
  ABS_ERROR_MEDIAN = median(ABS_ERROR_MEDIAN),
  DIR = median(DIR)
), by = "PROVINCE"]
historical_dir_rt_forecast_ptl_pred_dt[, list(
  ABS_ERROR_MEDIAN = median(ABS_ERROR_MEDIAN),
...
cor(
  historical_dir_rt_forecast_ptl_pred_dt$MEDIAN,
...
setnames(
  ...
  c(
    "CLIMATE_DIR_MEDIAN", "CLIMATE_DIR_CI_U", "CLIMATE_DIR_CI_L",
...
setnames(
  tmp,
  c(
    "UNDIFF_DIR_MEDIAN", "UNDIFF_DIR_CI_U", "UNDIFF_DIR_CI_L",
  ...
  c(
    "SARIMA_DIR_MEDIAN", "SARIMA_DIR_CI_U", "SARIMA_DIR_CI_L",
...
historical_log_scores <- scoringutils::logs_sample(
  historical_true_dirs,
  historical_climate_dir_pred_2010_2018
)
province_historical_log_scores_dt <- data.table(
  LOG_SCORE = historical_log_scores,
...
summary_province_historical_log_scores_dt_by_province_and_month <-
  province_historical_log_scores_dt[, list(
    MIN = quantile(LOG_SCORE, probs = 0.25),
...
historical_climate_sarima_dir_pred_2010_2018 <- cbind(
  historical_climate_dir_pred_2010_2018,
...
historical_climate_sarima_dir_pred_2010_2018_dt[, ITER := rep(
  rep(seq(1, 10000, by = 1), each = length(unique(PROVINCE))),
  length(unique(TIME))
)]
...
summary_historical_climate_sarima_dir_pred_2010_2018_dt <-
  historical_climate_sarima_dir_pred_2010_2018_dt[, list(
    MEDIAN = median(PRED),
...
binned_threshold_50_function <- function(thresholds, data) {
  ...
  for (i in 1:length(unique(thresholds))) {
    ...
    confusion_mat <- confusionMatrix(
      data = as.factor(data$OUTBREAK_50_PRED),
...
binned_threshold_50_function_with_ci <- function(thresholds, data) {
  ...
  for (i in 1:length(unique(thresholds))) {
    ...
    new_confusion_mat <- as.matrix(c(
      confusion_mat$table[2, 2],
      confusion_mat$table[1, 2],
      confusion_mat$table[2, 1],
      confusion_mat$table[1, 1]
    ), byrow = TRUE)
...
historical_climate_sarima_2010_2018_outbreak_50_results <- binned_threshold_50_function(
  thresholds = seq(0.05, 0.6, by = 0.005),
  summary_historical_climate_sarima_dir_pred_2010_2018_dt
)
...
climate_sarima_forecasting_2018_2021_outbreak_50_results <- binned_threshold_50_function(
  thresholds = historical_climate_sarima_2010_2018_chosen_threshold_50,
  summary_climate_sarima_dir_pred_2018_2021_dt
)
climate_sarima_forecasting_2018_2021_outbreak_50_results

climate_sarima_forecasting_2018_2021_outbreak_50_data <- binned_threshold_50_function_returning_data(
  thresholds = historical_climate_sarima_2010_2018_chosen_threshold_50,
  summary_climate_sarima_dir_pred_2018_2021_dt
)
...
climate_sarima_forecasting_2018_2021_outbreak_50_results_ci <-
  binned_threshold_50_function_with_ci(
    thresholds = historical_climate_sarima_2010_2018_chosen_threshold_50,
    summary_climate_sarima_dir_pred_2018_2021_dt
  )
climate_sarima_forecasting_2018_2021_outbreak_50_results_ci
...
binned_threshold_150_function <- function(thresholds, data) {
  ...
  for (i in 1:length(unique(thresholds))) {
    ...
    confusion_mat <- confusionMatrix(
      data = as.factor(data$OUTBREAK_150_PRED),
...
historical_climate_sarima_2010_2018_outbreak_150_results <- binned_threshold_150_function(
  thresholds = seq(0.05, 0.6, by = 0.005),
  summary_historical_climate_sarima_dir_pred_2010_2018_dt
)
...
climate_sarima_forecasting_2018_2021_outbreak_150_results <- binned_threshold_150_function(
  thresholds = historical_climate_sarima_2010_2018_chosen_threshold_150,
  summary_climate_sarima_dir_pred_2018_2021_dt
)
climate_sarima_forecasting_2018_2021_outbreak_150_results

climate_sarima_forecasting_2018_2021_outbreak_150_data <- binned_threshold_150_function_returning_data(
  thresholds = historical_climate_sarima_2010_2018_chosen_threshold_150,
  summary_climate_sarima_dir_pred_2018_2021_dt
)
climate_sarima_forecasting_2018_2021_outbreak_150_data

climate_sarima_forecasting_2018_2021_outbreak_150_results_ci <- binned_threshold_150_function_with_ci(
  thresholds = historical_climate_sarima_2010_2018_chosen_threshold_150,
  summary_climate_sarima_dir_pred_2018_2021_dt
)
...
confusion_mat_climate_sarima_first_outbreak_50 <- confusionMatrix(
  data = as.factor(climate_sarima_first_outbreak_50_dt$OUTBREAK_50_PRED),
...
roc_climate_sarima_first_outbreak_50 <- roc(
  climate_sarima_first_outbreak_50_dt$OUTBREAK_50_TRUE,
...
forecasting_sens_spec_climate_sarima_first_outbreak_50_table <- data.table(
  "Accuracy" = accuracy_climate_sarima_first_outbreak_50,
...
confusion_mat_climate_sarima_first_outbreak_150 <- confusionMatrix(
  data = as.factor(climate_sarima_first_outbreak_150_dt$OUTBREAK_150_PRED),
...
roc_climate_sarima_first_outbreak_150 <- roc(
  climate_sarima_first_outbreak_150_dt$OUTBREAK_150_TRUE,
...
forecasting_sens_spec_climate_sarima_first_outbreak_150_table <- data.table(
  "Accuracy" = accuracy_climate_sarima_first_outbreak_150,
...
climate_all_pit <- scoringutils::pit_sample(
  all_true_dirs,
  all_dir.pred
)
hist(climate_all_pit)
sarima_all_pit <- scoringutils::pit_sample(
  all_true_dirs,
  sarima_dir_pred_2018_2021
)
hist(sarima_all_pit)
ensemble_all_pit <- scoringutils::pit_sample(
  all_true_dirs,
...
pit_vals_dt <- data.table(
  ENSEMBLE = ensemble_all_pit,
...
new_climate_sarima_binned_weight_50_function <- function(weights, data, thresholds) {
  results_dt <- data.table(
    mod_weight = rep(weights, each = length(thresholds)),
