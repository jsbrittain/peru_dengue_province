...
province.python.out.dir <- file.path(province.base.dir, "python/output")
...
# data
log_info("Subset data")
ptl_province_2018_2021_data <- subset(ptl_province_inla_df, YEAR >= 2018)
...
process_python_deep_preds <- function(model_name) {
  ...
  for (i in 0:(length(province_names) - 1)) {
    ...
    tmp <- data.table(read.csv(file.path(province.python.out.dir, paste0(
      model_name,
      "_dataframe_", i, ".csv"
    ))))
...
ptl_province_2010_2018_data <- subset(ptl_province_inla_df, YEAR < 2018)
ptl_province_2010_2018_data[, IND := seq(1, length(DIR)), by = "PROVINCE"]
# If re-using this code, please note the hard-coded 82 below, based on the fact
# that we only have 42 data points in historical data.table
...
raw_historical_sarima_preds_dt[,
  IND := rep(seq(1, length(unique(ptl_province_2010_2018_data$TIME))), each = 1),
  by = "PROVINCE"]
head(raw_historical_sarima_preds_dt)

raw_historical_sarima_preds_dt$X <- NULL
historical_sarima_preds_dt <- melt(raw_historical_sarima_preds_dt, id.vars = c(
  "IND",
  "PROVINCE"
...
historical_sarima_preds_dt <- merge(historical_sarima_preds_dt, ptl_province_2010_2018_data,
  by = c("PROVINCE", "IND")
)
...
historical_sarima_preds_dt_for_scoring <- subset(historical_sarima_preds_dt, select = c(
  "location",
  "sample", "true_value", "prediction", "model", "end_of_month"
))
...
quantile_historical_sarima_preds_dt <- sample_to_quantile(historical_sarima_preds_dt_for_scoring,
  quantiles = c(0.01, 0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99)
)
process_quantile_predictions(quantile_historical_sarima_preds_dt)

quantile_historical_sarima_preds_dt[which(quantile == 0.5), caret::R2(
  prediction,
  true_value
), by = "location"]

saveRDS(quantile_historical_sarima_preds_dt, file = file.path(
  province.python.out.dir,
  "quantile_historical_sarima_preds_dt.RDS"
))
log_info("Saving sarima samples historical")
dir.create(file.path(
  province.predictions.out.dir,
  "sarima"
...
# setnames(historical_sarima_dir_preds_dt_for_scoring, 'SAMPLE', 'sample') #
# JSB: Changed _dt name
...
historical_sarima_dir_preds_dt_for_scoring <- subset(historical_sarima_dir_preds_dt_for_scoring,
  select = c("location", "sample", "true_value", "prediction", "model", "end_of_month")
)
...
quantile_historical_sarima_dir_preds_dt <- sample_to_quantile(historical_sarima_dir_preds_dt_for_scoring,
  quantiles = c(0.01, 0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99)
)
process_quantile_predictions(quantile_historical_sarima_dir_preds_dt)

# DIR
saveRDS(quantile_historical_sarima_dir_preds_dt, file = file.path(
  province.python.out.dir,
...
# Historical FINETUNED TIMEGPT---- 95, 90, 85, 80, 75, 70, 65, 60,
...
setnames(
  historical_fine_tuned_timegpt_preds_dt, colnames(historical_fine_tuned_timegpt_preds_dt)[4:26],
  tmp_quantiles
)
# historical_fine_tuned_timegpt_preds_dt[, IND:= seq(1, length(TimeGPT)), by =
# 'PROVINCE']
historical_fine_tuned_timegpt_preds_dt
ptl_province_2010_2018_data <- subset(ptl_province_inla_df, YEAR < 2018)
ptl_province_2010_2018_data <- subset(ptl_province_2010_2018_data, end_of_month >=
  min(historical_fine_tuned_timegpt_preds_dt$end_of_month))
# ptl_province_2010_2018_data[, IND:= seq(1, length(DIR)), by = 'PROVINCE']
historical_fine_tuned_timegpt_preds_dt
...
historical_fine_tuned_timegpt_preds_dt
historical_fine_tuned_timegpt_preds_dt <- merge(historical_fine_tuned_timegpt_preds_dt,
...
historical_fine_tuned_timegpt_preds_dt[, length(which(q_0.025 <= LOG_CASES & q_0.975 >=
  LOG_CASES)) / length(TimeGPT)]
...
tmp[, MEDIAN := NULL]
quantile_historical_fine_tuned_timegpt_preds_dt <- melt(tmp, id.vars = c(
  "PROVINCE",
  "LOG_CASES", "end_of_month", "POP_OFFSET"
), variable.name = "quantile", value.name = "prediction")
quantile_historical_fine_tuned_timegpt_preds_dt[, quantile := gsub("q_", "", quantile)]
quantile_historical_fine_tuned_timegpt_preds_dt[, quantile := gsub(
  "TimeGPT",
  0.5, quantile
)]
quantile_historical_fine_tuned_timegpt_preds_dt[, quantile := as.numeric(quantile)]
quantile_historical_fine_tuned_timegpt_preds_dt <- quantile_historical_fine_tuned_timegpt_preds_dt[which(quantile !=
  "TimeGPT")]
setnames(quantile_historical_fine_tuned_timegpt_preds_dt, c(
  "PROVINCE", "LOG_CASES",
  "end_of_month"
...
# quantile_historical_fine_tuned_timegpt_preds_dt[, prediction:=
# expm1(prediction)/POP_OFFSET]
# quantile_historical_fine_tuned_timegpt_preds_dt[, true_value:=
# expm1(true_value)/POP_OFFSET]
...
saveRDS(quantile_historical_fine_tuned_timegpt_preds_dt, file = file.path(
  province.python.out.dir,
...
quantile_historical_fine_tuned_timegpt_dir_preds_dt[which(prediction < 0), prediction :=
  0]
quantile_historical_fine_tuned_timegpt_dir_preds_dt[which(quantile == 0.5), caret::R2(
  prediction,
  true_value
)]
quantile_historical_fine_tuned_timegpt_dir_preds_dt
# DIR
saveRDS(quantile_historical_fine_tuned_timegpt_dir_preds_dt, file = file.path(
  province.python.out.dir,
  "quantile_historical_fine_tuned_timegpt_dir_preds_dt.RDS"
))
ggplot(historical_fine_tuned_timegpt_preds_dt) +
  geom_point(aes(
    x = TimeGPT, y = LOG_CASES,
    col = "Observed"
  )) +
  facet_wrap(PROVINCE ~ ., )
...
# Historical No covars FINETUNED TIMEGPT (USE)---- 95, 90, 85, 80, 75, 70, 65,
# 60,
...
setnames(
  historical_no_covars_fine_tuned_timegpt_preds_dt, colnames(historical_no_covars_fine_tuned_timegpt_preds_dt)[4:26],
  tmp_quantiles
)
ptl_province_2010_2018_data <- subset(ptl_province_inla_df, YEAR < 2018)
ptl_province_2010_2018_data <- subset(ptl_province_2010_2018_data, end_of_month >=
  min(historical_no_covars_fine_tuned_timegpt_preds_dt$end_of_month))
...
historical_no_covars_fine_tuned_timegpt_preds_dt <- merge(historical_no_covars_fine_tuned_timegpt_preds_dt,
  ptl_province_2010_2018_data,
  by = c("PROVINCE", "end_of_month")
)
...
historical_no_covars_fine_tuned_timegpt_preds_dt[, caret::MAE(
  expm1(TimeGPT) / POP_OFFSET,
  DIR
)]
historical_no_covars_fine_tuned_timegpt_preds_dt[, caret::R2(
  expm1(TimeGPT) / POP_OFFSET,
  DIR
)]
...
historical_no_covars_fine_tuned_timegpt_preds_dt[, length(which(q_0.025 <= LOG_CASES &
  q_0.975 >= LOG_CASES)) / length(TimeGPT)]

tmp <- copy(historical_no_covars_fine_tuned_timegpt_preds_dt)
colnames(tmp)[1:26]
tmp <- subset(tmp, select = c(colnames(tmp)[1:26], "LOG_CASES", "POP_OFFSET"))
tmp[, MEDIAN := NULL]
quantile_historical_no_covars_fine_tuned_timegpt_preds_dt <- melt(tmp, id.vars = c(
  "PROVINCE",
  "LOG_CASES", "end_of_month", "POP_OFFSET"
), variable.name = "quantile", value.name = "prediction")
quantile_historical_no_covars_fine_tuned_timegpt_preds_dt[, quantile := gsub(
  "q_",
  "", quantile
)]
quantile_historical_no_covars_fine_tuned_timegpt_preds_dt[, quantile := gsub(
...
quantile_historical_no_covars_fine_tuned_timegpt_preds_dt <- quantile_historical_no_covars_fine_tuned_timegpt_preds_dt[which(quantile !=
  "TimeGPT")]
setnames(quantile_historical_no_covars_fine_tuned_timegpt_preds_dt, c(
  "PROVINCE",
  "LOG_CASES", "end_of_month"
), c("location", "true_value", "target_end_date"))
...
saveRDS(quantile_historical_no_covars_fine_tuned_timegpt_preds_dt, file = file.path(
  province.python.out.dir,
...
quantile_historical_no_covars_fine_tuned_timegpt_dir_preds_dt[, prediction :=
  expm1(prediction) / POP_OFFSET]
quantile_historical_no_covars_fine_tuned_timegpt_dir_preds_dt[, true_value :=
  expm1(true_value) / POP_OFFSET]
quantile_historical_no_covars_fine_tuned_timegpt_dir_preds_dt[which(prediction <
  0), prediction := 0]
...
# DIR
saveRDS(quantile_historical_no_covars_fine_tuned_timegpt_dir_preds_dt, file = file.path(
  province.python.out.dir,
...
# historical_tcn_preds_dt <- melt(raw_historical_tcn_preds_dt, id.vars = )
# raw_historical_tcn_preds_dt$prediction <- NULL raw_historical_tcn_preds_dt
...
historical_tcn_preds_dt <- merge(historical_tcn_preds_dt, ptl_province_2010_2018_data,
  by = c("PROVINCE", "IND")
)
...
historical_tcn_preds_dt_for_scoring <- subset(historical_tcn_preds_dt, select = c(
  "location",
  "sample", "true_value", "prediction", "model", "end_of_month"
))
...
quantile_historical_tcn_preds_dt <- sample_to_quantile(historical_tcn_preds_dt_for_scoring,
  quantiles = c(0.01, 0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99)
)
...
saveRDS(quantile_historical_tcn_preds_dt, file = file.path(
  province.python.out.dir,
...
quantile_historical_tcn_dir_preds_dt <- sample_to_quantile(historical_tcn_dir_preds_dt_for_scoring,
  quantiles = c(0.01, 0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99)
)
...
quantile_historical_tcn_dir_preds_dt %>%
  ...
  summarise_scores(by = "model")
# DIR
saveRDS(quantile_historical_tcn_dir_preds_dt, file = file.path(
  province.python.out.dir,
...
# Testing Period (2018 - 2021) ---- NO COVARS TIMEGPT (USE) ----
...
# print(finetuned_no_covars_timegpt_preds_dt[, length(end_of_month), by =
# 'PROVINCE'])

finetuned_no_covars_timegpt_preds_dt <- merge(finetuned_no_covars_timegpt_preds_dt,
...
# by = 'PROVINCE']) print(finetuned_no_covars_timegpt_preds_dt[,
# caret::MAE(TimeGPT, LOG_CASES), by = 'PROVINCE'])
# print(finetuned_no_covars_timegpt_preds_dt[, caret::R2(TimeGPT, LOG_CASES)])
# print(finetuned_no_covars_timegpt_preds_dt[, caret::MAE(TimeGPT, LOG_CASES)])
# print(finetuned_no_covars_timegpt_preds_dt[, length(which(TimeGPT.q.2 <=
# LOG_CASES & TimeGPT.q.97 >= LOG_CASES)) / length(TimeGPT)])
...
tmp <- subset(tmp, select = c("PROVINCE", "IND", "end_of_month", paste0(
  "TimeGPT.q.",
  gpt_quantiles
), "YEAR", "MONTH", "TIME", "LOG_CASES", "POP_OFFSET"))
quantile_finetuned_no_covars_timegpt_preds_dt <- melt(tmp,
  id.vars = c(
    "PROVINCE",
    "IND", "TIME", "MONTH", "YEAR", "LOG_CASES", "end_of_month", "POP_OFFSET"
  ), variable.name = "quantile",
  value.name = "prediction"
)
quantile_finetuned_no_covars_timegpt_preds_dt <- quantile_finetuned_no_covars_timegpt_preds_dt[which(quantile !=
  "TimeGPT")]
setnames(quantile_finetuned_no_covars_timegpt_preds_dt, c(
  "PROVINCE", "LOG_CASES",
  "end_of_month"
), c("location", "true_value", "target_end_date"))

setkeyv(quantile_finetuned_no_covars_timegpt_preds_dt, "quantile")
num_repeats_for_each_quantile <- unique(quantile_finetuned_no_covars_timegpt_preds_dt[,
  length(IND),
  by = "quantile"
]$V1)
...
saveRDS(quantile_finetuned_no_covars_timegpt_preds_dt, file = file.path(
  province.python.out.dir,
...
quantile_fine_tuned_no_covars_timegpt_dir_preds_dt[which(prediction < 0), prediction :=
  0]
quantile_fine_tuned_no_covars_timegpt_dir_preds_dt[which(quantile == 0.5), caret::R2(
  prediction,
  true_value
)]
# DIR
saveRDS(quantile_fine_tuned_no_covars_timegpt_dir_preds_dt, file = file.path(
  province.python.out.dir,
  "quantile_fine_tuned_no_covars_timegpt_dir_preds_dt.RDS"
))

# quantile_finetuned_no_covars_timegpt_preds_dt[, prediction:=
# expm1(prediction)/POP_OFFSET] quantile_finetuned_no_covars_timegpt_preds_dt[,
# true_value:= expm1(true_value)/POP_OFFSET]
quantile_finetuned_no_covars_timegpt_preds_dt %>%
  score() %>%
  summarise_scores(by = c("model"))
...
fine_tuned_timegpt_preds_dt <- merge(fine_tuned_timegpt_preds_dt, ptl_province_2018_2021_data,
  by = c("PROVINCE", "IND", "end_of_month")
)
...
fine_tuned_timegpt_preds_dt[, length(which(TimeGPT.q.2 <= LOG_CASES & TimeGPT.q.97 >=
  LOG_CASES)) / length(TimeGPT)]
...
quantile_fine_tuned_timegpt_preds_dt <- melt(tmp,
  id.vars = c(
    "PROVINCE", "IND",
    "TIME", "MONTH", "YEAR", "LOG_CASES", "end_of_month", "POP_OFFSET"
  ), variable.name = "quantile",
  value.name = "prediction"
)
quantile_fine_tuned_timegpt_preds_dt <- quantile_fine_tuned_timegpt_preds_dt[which(quantile !=
  "TimeGPT")]
setnames(
...
num_repeats_for_each_quantile <- unique(quantile_fine_tuned_timegpt_preds_dt[, length(IND),
  by = "quantile"
]$V1)
num_repeats_for_each_quantile

quantile_fine_tuned_timegpt_preds_dt[, quantile := rep(c(0.01, 0.025, seq(
  0.05,
  0.95, 0.05
), 0.975, 0.99), each = num_repeats_for_each_quantile)]
...
saveRDS(quantile_fine_tuned_timegpt_preds_dt, file = file.path(
  province.python.out.dir,
...
quantile_fine_tuned_timegpt_dir_preds_dt[which(prediction < 0), prediction :=
  0]
quantile_fine_tuned_timegpt_dir_preds_dt[which(quantile == 0.5), caret::R2(
  prediction,
  true_value
)]

# DIR
saveRDS(quantile_fine_tuned_timegpt_dir_preds_dt, file = file.path(
  province.python.out.dir,
...
raw_tcn_preds_dt[, IND := rep(seq(1, length(unique(ptl_province_2018_2021_data$TIME))),
  each = 2500
), by = "PROVINCE"]
raw_tcn_preds_dt
raw_tcn_preds_dt <- merge(raw_tcn_preds_dt, ptl_province_2018_2021_data, by = c(
  "PROVINCE",
  "IND"
))
...
raw_tcn_preds_dt_for_scoring <- subset(raw_tcn_preds_dt, select = c(
  "location", "sample",
  "true_value", "prediction", "model", "end_of_month"
))
...
quantile_tcn_preds_dt <- sample_to_quantile(raw_tcn_preds_dt_for_scoring, quantiles = c(
  0.01,
...
saveRDS(quantile_tcn_preds_dt, file = file.path(province.python.out.dir, "quantile_tcn_preds_dt.RDS"))
...
dir_tcn_preds_dt_for_scoring <- subset(dir_tcn_preds_dt, select = c(
  "location", "sample",
  "DIR", "DIR_prediction", "model", "end_of_month"
))
...
# DIR
saveRDS(dir_tcn_preds_dt_for_scoring, file = file.path(
  province.python.out.dir,
...
quantile_tcn_dir_preds_dt <- sample_to_quantile(dir_tcn_preds_dt_for_scoring, quantiles = c(
  0.01,
  0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99
))
quantile_tcn_dir_preds_dt[which(quantile == 0.5), caret::R2(true_value, prediction)]
# DIR
saveRDS(quantile_tcn_dir_preds_dt, file = file.path(
  province.python.out.dir,
...
summary_tcn_preds_dt <- raw_tcn_preds_dt[, list(
  CI_L = quantile(prediction,
    probs = 0.025,
    na.rm = TRUE
  ), CI_U = quantile(prediction, probs = 0.975, na.rm = TRUE), MEDIAN = median(prediction,
    na.rm = TRUE
  ), MEAN = mean(prediction, na.rm = TRUE), true_value = unique(true_value),
  true_DIR = unique(DIR), DIR_CI_L = quantile(DIR_prediction, probs = 0.025, na.rm = TRUE),
  DIR_CI_U = quantile(DIR_prediction, probs = 0.975, na.rm = TRUE), DIR_MEDIAN = quantile(
    DIR_prediction,
    0.5
  ), OUTBREAK_PROB_50 = length(which(DIR_prediction >= 50 & !is.na(prediction))) / length(which(!is.na(prediction))),
  OUTBREAK_PROB_150 = length(which(DIR_prediction >= 150 & !is.na(prediction))) / length(which(!is.na(prediction)))
),
by = c("PROVINCE", "TIME", "YEAR", "model", "YEAR_DECIMAL")
]
...
# JSB: remove ggplot(summary_tcn_preds_dt) + geom_point(aes(x = TIME, y =
# DIR_MEDIAN), color = 'Prediction') + geom_point(aes(x = TIME, y = true_DIR),
# color = '')
...
raw_sarima_preds_dt[, IND := rep(seq(1, length(unique(ptl_province_2018_2021_data$TIME))),
  each = 2500
), by = "PROVINCE"]
raw_sarima_preds_dt
raw_sarima_preds_dt <- merge(raw_sarima_preds_dt, ptl_province_2018_2021_data, by = c(
  "PROVINCE",
  "IND"
))
...
raw_sarima_preds_dt_for_scoring <- subset(raw_sarima_preds_dt, select = c(
  "location",
  "sample", "true_value", "prediction", "model", "end_of_month"
))
...
quantile_sarima_preds_dt <- sample_to_quantile(raw_sarima_preds_dt_for_scoring, quantiles = c(
  0.01,
  0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99
))
...
quantile_sarima_preds_dt %>%
  ...
  summarise_scores(by = c("model"))
quantile_sarima_preds_dt
saveRDS(quantile_sarima_preds_dt, file = file.path(
  province.python.out.dir,
...
dir_sarima_preds_dt_for_scoring <- subset(dir_sarima_preds_dt, select = c(
  "location",
  "sample", "DIR", "DIR_prediction", "model", "end_of_month"
))
...
# DIR
saveRDS(dir_sarima_preds_dt_for_scoring, file = file.path(
  province.python.out.dir,
...
sarima_scores_dir_2018_2021_dt <- dir_sarima_preds_dt_for_scoring %>%
  score() %>%
  summarise_scores(by = c("model"))
quantile_sarima_dir_preds_dt <- sample_to_quantile(dir_sarima_preds_dt_for_scoring,
  quantiles = c(0.01, 0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99)
)
...
# DIR
saveRDS(quantile_sarima_dir_preds_dt, file = file.path(
  province.python.out.dir,
...
summary_sarima_preds_dt <- raw_sarima_preds_dt[, list(
  CI_L = quantile(prediction,
    probs = 0.025, na.rm = TRUE
  ), CI_U = quantile(prediction, probs = 0.975, na.rm = TRUE),
  MEDIAN = median(prediction, na.rm = TRUE), MEAN = mean(prediction, na.rm = TRUE),
  true_value = unique(true_value), true_DIR = unique(DIR), DIR_CI_L = quantile(DIR_prediction,
    probs = 0.025, na.rm = TRUE
  ), DIR_CI_U = quantile(DIR_prediction,
    probs = 0.975,
    na.rm = TRUE
  ), DIR_MEDIAN = median(DIR_prediction), OUTBREAK_PROB_50 = length(which(DIR_prediction >=
    50 & !is.na(prediction))) / length(which(!is.na(prediction))), OUTBREAK_PROB_150 = length(which(DIR_prediction >=
    150 & !is.na(prediction))) / length(which(!is.na(prediction)))
), by = c(
  "PROVINCE",
  "TIME", "YEAR", "model", "YEAR_DECIMAL"
)]
...
# JSB: Remove ggplot(summary_sarima_preds_dt) + geom_point(aes(x = TIME, y =
# DIR_MEDIAN), color = 'Prediction') + geom_point(aes(x = TIME, y = true_DIR),
# color = '')
