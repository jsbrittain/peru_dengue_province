...
if (file.exists(p01_filename)) {
    ...
    if (file.exists(p01a_filename)) {
        ...
        department_probable_confirmed_cases[which(REGION == "San Martin"), REGION :=
            "San Martín"]
        department_probable_confirmed_cases[which(REGION == "Junin"), REGION :=
            "Junín"]
        ...
        province_year_comb <- province_peru_cases[, .(PROVINCE, YEAR), by = c("PROVINCE",
            "YEAR")]
        # Create a data.table with all weeks from 1 to 52
        all_weeks <- data.table(week = 1:52)
        # Perform a cross join to get all possible combinations of
        ...
        all_combinations <- unique(CJ(PROVINCE = province_year_comb$PROVINCE, YEAR = province_year_comb$YEAR,
            WEEK = all_weeks$week))

        # Merge the original data.table with the full combinations to include
        # missing weeks
        log_info("Merge the original data.table with the full combinations to include missing weeks")
        filled_dt <- merge(all_combinations, province_peru_cases, by = c("PROVINCE",
            "YEAR", "WEEK"), all.x = TRUE)
        ...
        setnames(province_peru_pops, colnames(province_peru_pops)[3:6], c("11/07/1993",
        ...
        fill_province_pops_dt <- function(data) {
            ...
            filled_pops <- data.table(YEAR = rep(seq(min_yr, max_yr, by = 1), each = 12 *
                ...
                2022, by = 1)) * length(unique(data$PROVINCE))))
            return(filled_pops)
        }

        province_peru_pops <- melt(province_peru_pops, id.vars = "PROVINCE", variable.name = "YEAR",
            value.name = "POP")
        province_peru_pops <- data.table(province_peru_pops)
        province_peru_pops[, MONTH := as.numeric(as.character(substr(YEAR, 4,
            5)))]
        ...
        province_peru_pops_filled[, PROVINCE := rep(unique(province_peru_pops$PROVINCE),
        ...
        for (i in 1:length(unique(province_peru_pops_filled$PROVINCE))) {
            ...
            province_peru_pops_filled[which(PROVINCE == rgn_in_q & TIME <= 183),
                POP := tmp2]
        }
        ...
        for (i in 1:nrow(tmp_names)) {
            closest_match <- unique(peru_district_boundaries2$ADM2_ES)[which.min(stringdist::stringdist(unique(tmp_names$PROVINCE)[i],
        ...
        monthly_province_peru_cases <- merge(monthly_province_peru_cases, tmp, by = c("PROVINCE",
            "YEAR", "MONTH"))
                  ...
                  for (j in 1:12) {
                    ...
                    if (file.exists(rds_file)) {
                      ...
                      tmp_climate_provinces <- (raster::extract(tmp_climate_var_peru,
                        peru_district_boundaries2, fun = "mean", weights = TRUE,
                        na.rm = TRUE))
        ...
        spi_data_table_func <- function() {
            ...
            for (i in 1:length(unique(spi_dt$YEAR))) {
                ...
                tmp$spi <- data.table(raster::extract(r, tmp, fun = "mean", weights = TRUE,
                  na.rm = TRUE))
    ...
    # ::: JSB fix ::: icen_data <-
    # data.table(read.table(file.path(peru.province.base.dir, 'icen',
    # 'icen.txt'), header = TRUE))
    icen_data <- data.table(read.table(file.path(peru.province.base.dir, "icen",
        "ICEN.txt"), header = FALSE, comment.char = "%"))
    ...
    climate_dt_province <- merge(climate_dt_province, prec_2010_2019_province, by = c("PROVINCE",
        "YEAR", "TIME", "MONTH"))
    climate_dt_province <- merge(climate_dt_province, spi_province_dt, by = c("PROVINCE",
        "YEAR", "MONTH"))
    ...
    climate_20_21_province <- merge(climate_20_21_province, tmin_2020_2021_province,
        by = c("PROVINCE", "YEAR", "TIME", "MONTH"))
    climate_20_21_province <- merge(climate_20_21_province, icen_data, by = c("YEAR",
        "MONTH"))
    climate_20_21_province <- merge(climate_20_21_province, oni_data, by = c("YEAR",
        "MONTH"))
    climate_20_21_province <- merge(climate_20_21_province, spi_province_dt, by = c("PROVINCE",
        "YEAR", "MONTH"))
    ...
    # climate_dt_province <- merge(climate_dt_province, spei_province_dt, by =
    # c( 'PROVINCE', 'TIME' ))
    ...
    peru_2007_census_province_urban_propns <- as.numeric(peru_2007_census_data[grepl("Urbano",
        peru_2007_census_data$AREA_TYPE), ]$PROPN)
    peru_2007_census_province_rural_propns <- as.numeric(peru_2007_census_data[grepl("Rural",
        peru_2007_census_data$AREA_TYPE), ]$PROPN)
    ...
    peru_2007_census_province_rural_propns <- c(peru_2007_census_province_rural_propns[1:66],
    ...
    peru_2007_census_data <- data.table(PROVINCE = peru_2007_census_province_names,
    ...
    raw_peru_2017_census_data <- data.table(readxl::read_excel(file.path(peru.province.data.dir,
        "census_data.xlsx"), sheet = 1, skip = 2))
    peru_2017_census_data <- copy(raw_peru_2017_census_data)
    peru_2017_census_data <- subset(peru_2017_census_data, select = colnames(peru_2017_census_data)[c(1,
        2, 5, 8)])
    ...
    for (i in 1:nrow(tmp_monthly_province_peru_cases_names)) {
        closest_match <- unique(tmp_peru_2007_census_names$PROVINCE)[which.min(stringdist::stringdist(unique(tmp_monthly_province_peru_cases_names$PROVINCE)[i],
    ...
    monthly_province_peru_cases <- merge(monthly_province_peru_cases, subset(tmp,
        select = c("PROVINCE", "PROPN_URBAN_2007")), by = "PROVINCE")
    ...
    tmp_monthly_province_peru_cases_names <- unique(subset(monthly_province_peru_cases,
        select = c("PROVINCE")))
    for (i in 1:nrow(tmp_monthly_province_peru_cases_names)) {
        closest_match <- unique(tmp_peru_2017_census_names$PROVINCE)[which.min(stringdist::stringdist(unique(tmp_monthly_province_peru_cases_names$PROVINCE)[i],
    ...
    monthly_province_peru_cases <- merge(monthly_province_peru_cases, subset(tmp,
        select = c("PROVINCE", "PROPN_URBAN_2017")), by = "PROVINCE")
    ...
    monthly_province_peru_cases[, DIFF_WITH_HISTORICAL_DIR := DIR - HISTORICAL_DIR,
        by = "PROVINCE"]
    monthly_province_peru_cases[, DIFF_WITH_HISTORICAL_DIR_LAG := Lag(DIFF_WITH_HISTORICAL_DIR,
        1), by = "PROVINCE"]
    ...
    # CCFs of climatic variables + DIR during model development phase
    # (pre-2018) ----
    log_info("CCFs of climatic variables + DIR during model development phase (pre-2018)")

    # tmax CCF ----
    tmp <- monthly_province_peru_cases[which(!is.na(tmax) & !is.na(DIR) & YEAR <
        2018)]
    ...
    for (i in 1:length(tmax_ccf_province_list)) {
        ...
        if (length(tmp_acf) > 0) {
            tmp_acf_dt <- data.table(LAG = tmp_acf$lag, ACF = tmp_acf$acf, PROVINCE = rep(unique(tmp$PROVINCE)[i],
                35))
            for (j in 1:length(unique(tmp_acf_dt$LAG))) {
                lag <- unique(tmp_acf_dt$LAG)[j]
                tmax_ccf_province_dt[which(PROVINCE == unique(tmp$PROVINCE)[i] &
                  LAG == lag), ACF := tmp_acf$acf[j]]
            }
        }
    }
    ...
    # tmin
    tmp <- monthly_province_peru_cases[which(!is.na(tmin) & !is.na(DIR) & YEAR <
        2018)]
    tmin_ccf_province_dt <- data.table(PROVINCE = rep(unique(tmp$PROVINCE), 35),
        LAG = rep(seq(-17, 17, by = 1), each = length(unique(tmp$PROVINCE))))
    # tmin_ccf_province_list <- tmp[which(!is.na(tmin) & !is.na(DIR)),
    # ccf(tmin, DIR, na.rm = TRUE, plot = FALSE)$acf, by = c('PROVINCE')]
    ...
    for (i in 1:length(tmin_ccf_province_list)) {
        ...
        if (length(tmp_acf) > 0) {
            ...
            for (j in 1:length(unique(tmp_acf_dt$LAG))) {
                lag <- unique(tmp_acf_dt$LAG)[j]
                tmin_ccf_province_dt[which(PROVINCE == unique(tmp$PROVINCE)[i] &
                  LAG == lag), ACF := tmp_acf$acf[j]]
            }
        }
    }
    ...
    # Prec CCF ----
    log_info("Prec CCF")
    tmp <- monthly_province_peru_cases[which(!is.na(prec) & !is.na(DIR) & YEAR <
        2018)]
    ...
    for (i in 1:length(prec_ccf_province_list)) {
        ...
        if (length(tmp_acf) > 0) {
            ...
            for (j in 1:length(unique(tmp_acf_dt$LAG))) {
                lag <- unique(tmp_acf_dt$LAG)[j]
                prec_ccf_province_dt[which(PROVINCE == unique(tmp$PROVINCE)[i] &
                  LAG == lag), ACF := tmp_acf$acf[j]]
            }
        }
    }
    ...
    # SPI CCF----
    log_info("SPI CCF")
    tmp <- monthly_province_peru_cases[which(!is.na(SPI_6) & !is.na(DIR) & YEAR <
        2018)]
    spi_ccf_province_dt <- data.table(PROVINCE = rep(unique(tmp$PROVINCE), 35), LAG = rep(seq(-17,
        17, by = 1), each = length(unique(tmp$PROVINCE))))
    ...
    # ONI CCF ----
    log_info("ONI CCF")
    tmp <- monthly_province_peru_cases[which(!is.na(ANOM) & !is.na(DIR) & YEAR <
        2018)]
    ...
    # ICEN CCF ----
    log_info("ICEN CCF")
    tmp <- monthly_province_peru_cases[which(!is.na(E_INDEX) & !is.na(DIR) & YEAR <
        2018)]
    ...
    for (i in 1:length(icen_ccf_province_list)) {
        ...
        if (length(tmp_acf) > 0) {
            ...
            for (j in 1:length(unique(tmp_acf_dt$LAG))) {
                lag <- unique(tmp_acf_dt$LAG)[j]
                icen_ccf_province_dt[which(PROVINCE == unique(tmp$PROVINCE)[i] &
                  LAG == lag), ACF := tmp_acf$acf[j]]
            }
        }
    }
    ...
    tmp_monthly_province_peru_cases_names <- unique(subset(tmp_monthly_province_peru_cases,
        select = c("PROVINCE")))
    for (i in 1:nrow(tmp_monthly_province_peru_cases_names)) {
        closest_match <- unique(province_peru_dt$PROVINCIA)[which.min(stringdist::stringdist(unique(tmp_monthly_province_peru_cases_names$PROVINCE)[i],
    ...
    tmp_monthly_province_peru_cases <- merge(tmp_monthly_province_peru_cases, subset(tmp_monthly_province_peru_cases_names,
        select = c("PROVINCE", "coords_x", "coords_y")), by = "PROVINCE")
    tmp_monthly_province_peru_cases_names
    setnames(tmp_monthly_province_peru_cases, c("coords_x", "coords_y"), c("longitude",
        "latitude"))
    monthly_province_peru_cases
    monthly_province_peru_cases <- merge(monthly_province_peru_cases, unique(subset(tmp_monthly_province_peru_cases,
        select = c("PROVINCE", "longitude", "latitude"))), by = "PROVINCE")
