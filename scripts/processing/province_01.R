...
all_combinations <- CJ(
  PROVINCE = province_year_comb$PROVINCE,
  CASE_TYPE = case_types$V1,
  YEAR = province_year_comb$YEAR,
  WEEK = all_weeks$week
)
...
monthly_department_probable_confirmed_cases <- as.data.table(
  monthly_department_probable_confirmed_cases
)
...
monthly_department_probable_confirmed_cases[
  which(TOTAL_CASES != 0),
  PROPN_CONFIRMED := CONFIRMED / TOTAL_CASES
]
monthly_department_probable_confirmed_cases[
  which(TOTAL_CASES == 0),
  PROPN_CONFIRMED := 0
...
district_peru_cases <- raw_peru_cases[
  ,
  list(TOTAL_CASES = length(Sexo)),
...
setnames(
  district_peru_cases,
  colnames(district_peru_cases),
  c("DISTRICT", "PROVINCE", "REGION", "WEEK", "YEAR", "TOTAL_CASES")
)
...
missing_regions_from_cases <- unique(district_peru_cases$REGION)[
  which(!(unique(district_peru_cases$REGION) %in% peru_district_boundaries2$ADM1_ES))
]
...
province_year_comb <- province_peru_cases[
  ,
  .(PROVINCE, YEAR),
  by = c("PROVINCE", "YEAR")
]
...
raw_province_peru_pops <- data.table(
  read.csv(
    file.path(
...
setnames(
  province_peru_pops, colnames(province_peru_pops)[3:6],
...
# :::JSB::: after running the above, call extract_worldclim.R which contains the following code
# modified to save each month as they are processed. This needs incorporating into the main script
# and/or setting up in a workflow manager to run in parallel and more efficiently.

# The following code is a copy of the code in extract_worldclim.R and should reference that code
...
if (TRUE) {
  ...
  extract_worldclim_variable_provinces_2010_2019 <- function(years_sequence, climate_variable) {
    worldclim_variable_dt <- data.table(
      YEAR = rep(years_sequence, each = 12 * length(region_province$PROVINCE)),
    ...
    for (i in 1:length(years_sequence)) {
      ...
      for (j in 1:12) {
        rds_file = paste0(tools::file_path_sans_ext(climate_variable_monthly_files[j]), '.RDS')
        ...
        worldclim_variable_dt[which(YEAR == year & MONTH == j), VALUE := rep(tmp_climate_provinces, length.out = .N)]
      }
    }
...
spi_data_table_func <- function() {
  spi_dt <- data.table(
    YEAR = rep(seq(2001, 2022, by = 1), each = length(peru_district_boundaries2$ADM2_ES) * 12),
...
raw_peru_2007_census_data <- data.table(readxl::read_excel(
  file.path(
    peru.province.data.dir,
...
peru_2007_census_province_rural_propns <-
  c(
    peru_2007_census_province_rural_propns[1:66], 0,
...
raw_peru_2017_census_data <- data.table(readxl::read_excel(
  file.path(
    peru.province.data.dir,
    "census_data.xlsx"
  ),
...
median(monthly_province_peru_cases[which(!is.na(DIFF_WITH_HISTORICAL_DIR_LAG) & DIR > 0),
  cor(DIFF_WITH_HISTORICAL_DIR_LAG, DIR),
  by = "PROVINCE"
]$V1, na.rm = TRUE)
...
tmax_ccf_province_dt <- data.table(
  PROVINCE = rep(unique(tmp$PROVINCE), 35),
...
for (i in 1:length(unique(tmp$PROVINCE))) {
  ...
  if (nrow(tmp2) > 1) {
    tmax_ccf_province_list[[i]] <- ccf(tmp2$tmax, tmp2$DIR,
      na.rm = TRUE,
      plot = FALSE
    )
  }
...
all_peru_tmax_ccf_facet_plot <- ggplot(tmax_ccf_province_dt2) +
  geom_line(aes(x = LAG, y = ACF, color = PROVINCE)) +
...
tmin_ccf_province_dt <- data.table(
  PROVINCE = rep(unique(tmp$PROVINCE), 35),
  LAG = rep(seq(-17, 17, by = 1), each = length(unique(tmp$PROVINCE)))
)
...
for (i in 1:length(unique(tmp$PROVINCE))) {
  tmp2 <- subset(tmp, PROVINCE == unique(tmp$PROVINCE)[i])
  if (nrow(tmp2) > 1) {
    tmin_ccf_province_list[[i]] <- ccf(tmp2$tmin, tmp2$DIR,
      na.rm = TRUE,
      plot = FALSE
    )
  }
...
prec_ccf_province_dt <- data.table(
  PROVINCE = rep(unique(tmp$PROVINCE), 35),
  LAG = rep(seq(-17, 17, by = 1), each = length(unique(tmp$PROVINCE)))
)
...
for (i in 1:length(unique(tmp$PROVINCE))) {
  tmp2 <- subset(tmp, PROVINCE == unique(tmp$PROVINCE)[i])
  if (nrow(tmp2) > 1) {
    prec_ccf_province_list[[i]] <- ccf(tmp2$prec, tmp2$DIR,
      na.rm = TRUE,
      plot = FALSE
    )
  }
...
spi_ccf_province_dt <- data.table(
  PROVINCE = rep(unique(tmp$PROVINCE), 35),
  LAG = rep(seq(-17, 17, by = 1), each = length(unique(tmp$PROVINCE)))
)
...
for (i in 1:length(unique(tmp$PROVINCE))) {
  tmp2 <- subset(tmp, PROVINCE == unique(tmp$PROVINCE)[i])
  if (nrow(tmp2) > 1) {
    spi_ccf_province_list[[i]] <- ccf(tmp2$SPI_6, tmp2$DIR,
      na.rm = TRUE,
      plot = FALSE
    )
  }
...
oni_ccf_province_dt <- data.table(
  PROVINCE = rep(unique(tmp$PROVINCE), 35),
  LAG = rep(seq(-17, 17, by = 1), each = length(unique(tmp$PROVINCE)))
)
...
for (i in 1:length(unique(tmp$PROVINCE))) {
  tmp2 <- subset(tmp, PROVINCE == unique(tmp$PROVINCE)[i])
  if (nrow(tmp2) > 1) {
    oni_ccf_province_list[[i]] <- ccf(tmp2$ANOM, tmp2$DIR,
      na.rm = TRUE,
      plot = FALSE
    )
  }
...
icen_ccf_province_dt <- data.table(
  PROVINCE = rep(unique(tmp$PROVINCE), 35),
  LAG = rep(seq(-17, 17, by = 1), each = length(unique(tmp$PROVINCE)))
)
...
tmp_monthly_province_peru_cases_names <- merge(tmp_monthly_province_peru_cases_names, subset(province_peru_dt, select = c(
  "coords_x", "coords_y",
  "PROVINCIA"
)),
...
tmp_monthly_province_peru_cases_names
setnames(
  tmp_monthly_province_peru_cases, c("coords_x", "coords_y"),
