...
if (file.exists(p02_filename)) {
  ...
  load(file = p02_filename)
    ...
    piura_tumbes_lambayeque_boundaries_with_cases <- subset(
      piura_tumbes_lambayeque_boundaries,
    ...
    nb2INLA(
      file.path(peru.province.inla.data.in.dir, "nbr_piura_tumbes_lambayeque.graph"),
    ...
    monthly_province_peru_cases <- merge(monthly_province_peru_cases, tmp2, by = c(
      "PROVINCE",
      "TIME"
    ))
    ...
    # Only keep from Time Point 5 (May 2010) onwards - Reason = We use a 3-month
    # period to evaluate momentum (RSI) and the RSI predictor is lagged by 1 month
    # -> May RSI predictor is the value from April
    ...
    # Setting up data.table used in wavelet analysis, climate-based modelling, and
    # forecasting ---- For model development, we will subset this data.table to
    # only include data between 2010 and 2017 (inclusive)
    ...
    ptl_province_peru_dt <- merge(ptl_province_peru_dt, unique(subset(ptl_province_inla_df,
      select = c("PROPN_URBAN_2007", "PROPN_URBAN_2017", "PROVINCE")
    )),
    by.x = "PROVINCIA",
    by.y = "PROVINCE"
    )
