...
province.analysis.out.dir <- file.path(getwd(), "analysis")
...
quantile_fine_tuned_timegpt_preds_dt <- subset(
  quantile_fine_tuned_timegpt_preds_dt, select = scoring_columns)
...
quantile_finetuned_no_covars_timegpt_preds_dt <- subset(
  quantile_finetuned_no_covars_timegpt_preds_dt, select = scoring_columns)

# 5) Climate
log_info("Load climate")
climate_2018_2021_log_cases_quantile_dt <- read.csv(
  file = file.path(
    province.predictions.out.dir, "bayesian",
    paste0("pred_", metric_name, "_quantiles_forecasting.csv")))
climate_2018_2021_log_cases_quantile_dt <- merge(
...
climate_2018_2021_log_cases_quantile_dt <- subset(
  climate_2018_2021_log_cases_quantile_dt,
  select = scoring_columns
)
...
iteratively_train_quantile_ensemble <- function(
  quantiles,
  ...
  # ovr_num_pred_points = total number points (includes quantile predictions)
  ...
  for (t in 1:number_testing_dates) {
    ...
    # JSB: Iteratively adds rows to analysis df
      ...
      for (j in 1:num_ensemble_components) {
        qarr[i, j, ] <-
          historical_input_quantile_dt[which(IND == prediction_indices[i] &
            model == model_names[j]), ]$prediction
      }
    }
    ...
    model_weights_dt <- data.table(
      model = model_names, weights = quantile_ensemble_weights$alpha)
    ...
    model_weights_dt[,
      target_end_date := unique(testing_input_quantile_dt$target_end_date)]
...
iteratively_train_quantile_ensemble_parallel <- function(
  quantiles,
  number_testing_dates,
  input_quantile_dt,
  pred_points_dt,
  ...
  # Parallel loop
  results <- foreach(
    t = 1:number_testing_dates,
    .combine = rbind,
    .packages = c("data.table", "logger"),
    .export = c("quantile_ensemble")
  ) %dopar% {
    log_info(paste0(
      "iteratively_train_quantile_ensemble (", testing_dates[t],
      ", t = ", t, " of ", number_testing_dates, ")"
...
saveRDS(iteratively_trained_weights_log_cases_dt,
  file = file.path(province.out.dir, paste0("iteratively_trained_weights_", metric_name, "_dt.RDS"))
)
...
weighted_quantile_log_cases_forecasts_2018_2021_dt[
  which(quantile == 0.5), caret::R2(prediction, true_value)]
weighted_quantile_log_cases_forecasts_2018_2021_dt[
  which(quantile == 0.5), caret::MAE(prediction, true_value)]
weighted_quantile_log_cases_forecasts_2018_2021_dt[,
  model := "pinball_trained_ensemble"]

# 2) Province-dependent
log_info("Province-dependent")
iteratively_trained_weights_dt_by_province <-
  iteratively_train_quantile_ensemble_by_province(
    quantiles = quantiles,
    number_testing_dates = number_testing_dates,
    input_quantile_dt = input_quantile_dt,
...
wis_expanding_window_over_time <- function(models_dt, times) {
...
dir.create(file.path(
  province.analysis.out.dir,
  "wis"
), recursive = TRUE, showWarnings = FALSE)
write.csv(wis_summary,
  file.path(province.analysis.out.dir, "wis", "wis_summary.csv"),
  row.names = FALSE
)
log_info("Writing WIS over time (rolling window) - Provinces...")
write.csv(wis_province,
  file.path(province.analysis.out.dir, "wis", "wis_province.csv"),
  row.names = FALSE
)
...
# # 
# # 
# iteratively_train_quantile_ensemble_runner <- function(
#   quantiles,
#   dates,
#   lagged_dates,
#   number_dates,
...
#     historical_input_quantile_dt[, IND := NULL]
...
#   input_quantile_dt[,
#     list(prediction = sum(weights * prediction),
#          true_value = unique(true_value)),
#     by = c("location", "quantile", "target_end_date")]
# trained_ensemble_quantile_log_cases_forecasts_2018_2021_dt[
#   which(quantile == 0.5), caret::R2(prediction, true_value)]
...
#   file = file.path(province.out.dir, paste0("runner_trained_ensemble_weights_by_province_quantile_", metric_name, "_dt.RDS"))
# )
# 
...
#   which(quantile == 0.5), caret::MAE(prediction, true_value)]
# trained_ensemble_by_province_quantile_log_cases_forecasts_2018_2021_dt[,
#   model := "pinball_trained_ensemble_by_province"]
# 
# trained_ensemble_by_province_quantile_log_cases_forecasts_2018_2021_dt <-
#   fix_quantile_violations(
#     trained_ensemble_by_province_quantile_log_cases_forecasts_2018_2021_dt)
...
# FIX NAMES ----
original_model_names <- sort(
  unique(quantile_log_cases_components_plus_ensembles_dt$model))
new_model_names <- c(
  "Baseline", "Bayes-Climate", "EW-Mean *", "EW-Mean-NoBase *",
  "Ew-Mean-NoBayes *", "EW-Mean-NoCov *", "TimeGPT",
  "TimeGPT-NoCov ", "Median *", "Median-NoBase *", "Median-NoBayes *",
  "Median-NoCov *", "Trained *", "Prov-Trained *", "SARIMA", "TCN"
)
# historical_original_model_names <- sort(
#   unique(historical_quantile_log_cases_components_plus_ensembles_dt$model))
...
for (name in historical_new_model_names) {
  ...
  write.csv(tmp,
    file.path(province.predictions.out.dir, model_name,
      paste0("pred_", metric_name, "_quantiles_forecasting.csv")),
    row.names = FALSE
  )
  ...
  # write.csv(tmp,
  #   file.path(province.predictions.out.dir, model_name,
  #     paste0("pred_", metric_name, "_quantiles_historical.csv")),
  #   row.names = FALSE
  # )
}
