...
prov.rank.multi <- function(df2, df1, pval.cut) {
  ...
  corr.prov <- analyze.coherency(df.merge,
    my.pair = c("DIR_PROV_IN_Q", "DIR_PROV_OTHER"),
    loess.span = 0,
    dt = 1 / 12, dj = 1 / 100,
...
complete.coherence.df[
  which(coherence_prov == "Contralmirante_Villar"),
...
greatest_propn_coherent_df <- significance_coherence_df[, list(
  AVG_PAIRWISE_COHERENCE = mean(coherence, na.rm = TRUE),
...
annual_greatest_propn_coherent_df <- subset(
  greatest_propn_coherent_df,
  cycle_type == "annual"
)
...
ptl_province_peru_map_propn_annual_coherent <-
  ...
  geom_sf(aes(fill = propn_annual_coherent), alpha = 0.7) +
  theme_bw() +
...
multi_greatest_propn_coherent_df <- subset(
  greatest_propn_coherent_df,
  cycle_type == "multi"
)
...
ptl_province_peru_map_propn_multi_coherent <-
  ggplot(ptl_province_peru_dt, aes(geometry = geometry)) +
  geom_sf(aes(fill = propn_multi_coherent), alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Longitude") +
  ylab("Latitude") +
...
pairwise_significance_coherence_df <-
  significance_coherence_df[, list(
    AVG_PAIRWISE_COHERENCE = mean(coherence, na.rm = TRUE),
    PROPN_SIGNIFICANT_COHERENT =
      length(which(sig_coherence == 1)) / length(sig_coherence)
  ),
...
setnames(tmp, c(
  "coherence_LAT_PROV_IND", "coherence_LONG_PROV_IND",
  "coherence_prov"
))
pairwise_significance_coherence_df <- merge(pairwise_significance_coherence_df,
  tmp,
...
annual_pairwise_significance_coherence_plot <- ggplot(annual_pairwise_significance_coherence_df) +
  geom_tile(aes(
    x = LAT_PROV_IND, y = coherence_LAT_PROV_IND,
    fill = PROPN_SIGNIFICANT_COHERENT,
    color = PROPN_SIGNIFICANT_COHERENT
  )) +
  theme_bw() +
  labs(x = "Province", y = "Province") +
  theme(
    text = element_text(size = 28),
...
ptl_province_peru_map_corr_betw_dist_and_propn_significant_coherent <-
  ggplot(ptl_province_peru_dt, aes(geometry = geometry)) +
  geom_sf(aes(fill = corr_betw_dist_and_propn_significant_coherent), alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Longitude") +
  ylab("Latitude") +
...
ptl_province_peru_map_corr_betw_dist_and_propn_significant_coherent_multi <-
  ggplot(ptl_province_peru_dt, aes(geometry = geometry)) +
  geom_sf(aes(fill = corr_betw_dist_and_propn_significant_coherent_multi), alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Longitude") +
  ylab("Latitude") +
