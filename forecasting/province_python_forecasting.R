...
process_python_deep_preds <- function(model_name){
  ...
  for(i in 0:13){
    ...
    tmp <- 
      data.table(read.csv(file.path(peru.province.python.out.dir,
                                    paste0(model_name,
                                           "_dataframe_",i,
                                           ".csv"))))
...
raw_historical_sarima_preds_dt[, IND:= rep(seq(1, 
...
historical_sarima_preds_dt <- melt(raw_historical_sarima_preds_dt, id.vars = c("IND", "PROVINCE"),
...
saveRDS(quantile_historical_sarima_preds_dt,
        file = file.path(peru.province.python.out.dir,
...
setnames(historical_fine_tuned_timegpt_preds_dt,
...
ptl_province_2010_2018_data <- subset(ptl_province_inla_df,
                                      YEAR < 2018)
ptl_province_2010_2018_data <- subset(ptl_province_2010_2018_data,
...
tmp[, MEDIAN:= NULL]
quantile_historical_fine_tuned_timegpt_preds_dt <- melt(tmp, id.vars = c("PROVINCE",  "LOG_CASES", "end_of_month", "POP_OFFSET"),
...
setnames(quantile_historical_fine_tuned_timegpt_preds_dt, c("PROVINCE", "LOG_CASES", "end_of_month"), 
...
tmp_quantiles <- tmp_quantiles[which(tmp_quantiles!= 0.5)]
tmp_quantiles[1:11]<- rev.default(tmp_quantiles[1:11])
setnames(historical_no_covars_fine_tuned_timegpt_preds_dt,
...
tmp <- subset(tmp, select = c(colnames(tmp)[1:26], "LOG_CASES", "POP_OFFSET"))
tmp[, MEDIAN:= NULL]
quantile_historical_no_covars_fine_tuned_timegpt_preds_dt <- melt(tmp, id.vars = c("PROVINCE",  "LOG_CASES", "end_of_month", "POP_OFFSET"),
                                                        variable.name = "quantile", value.name = "prediction")
...
setnames(quantile_historical_no_covars_fine_tuned_timegpt_preds_dt, c("PROVINCE", "LOG_CASES", "end_of_month"), 
         c("location", "true_value","target_end_date"))
...
ptl_province_2010_2018_data[, IND:= seq(1, length(DIR)), by = "PROVINCE"] #New Index to match forecast data.table
raw_historical_tcn_preds_dt[, IND:= rep(seq(1, 
                                            length(unique(ptl_province_2010_2018_data$TIME))),
                                        each = 1),
                            by = "PROVINCE"]
...
quantile_historical_tcn_preds_dt %>% score() %>% summarise_scores(by = "model")
...
quantile_historical_tcn_dir_preds_dt %>% score() %>% summarise_scores(by = "model")
...
quantile_finetuned_no_covars_timegpt_preds_dt <- melt(tmp, id.vars = c("PROVINCE", "IND", "TIME", 
...
setnames(quantile_finetuned_no_covars_timegpt_preds_dt, c("PROVINCE", "LOG_CASES", "end_of_month"), 
         c("location", "true_value","target_end_date"))
...
colnames(tmp)[1:27]
tmp <- subset(tmp, select = c(colnames(tmp)[1:30], "LOG_CASES", "POP_OFFSET"))
quantile_fine_tuned_timegpt_preds_dt <- melt(tmp, id.vars = c("PROVINCE", "IND", "TIME", 
...
quantile_fine_tuned_timegpt_preds_dt[, quantile:= rep(c(0.01, 0.025, 
                                                      seq(0.05, 0.95, 0.05), 
                                                      0.975, 0.99),each = num_repeats_for_each_quantile)]
...
raw_tcn_preds_dt[, IND:= rep(seq(1, 
...
summary_tcn_preds_dt <- 
  raw_tcn_preds_dt[, list(CI_L = quantile(prediction, probs = 0.025, na.rm = TRUE),
...
raw_sarima_preds_dt[, IND:= rep(seq(1, 
                                 length(unique(ptl_province_2018_2021_data$TIME))),
                             each = 2500),
                 by = "PROVINCE"]
raw_sarima_preds_dt
...
summary_sarima_preds_dt <- 
  raw_sarima_preds_dt[, list(CI_L = quantile(prediction, probs = 0.025, na.rm = TRUE),
                          CI_U = quantile(prediction,  probs = 0.975, na.rm = TRUE),
                          MEDIAN = median(prediction, na.rm = TRUE),
                          MEAN = mean(prediction, na.rm = TRUE),
                          true_value = unique(true_value), 
