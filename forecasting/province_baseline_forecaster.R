...
fit_baseline_function_to_all_data <- function(data, predict_times){
  ...
  for(i in 1:length(unique(data$PROVINCE))){
    #Baseline model fitted to each province independently
    if(i %% 5 == 0){
      print(i) #Memory considerations
      gc()}
    ...
    for(j in 1:length(predict_times)){
      ...
      tmp_province_level_data <- subset(province_level_data, 
...
baseline_results <- fit_baseline_function_to_all_data(ptl_province_inla_df,
                                                      predict_times)
saveRDS(baseline_results,
        file = file.path(peru.province.ensemble.out.dir,
                         "baseline_results_2010_2021.RDS"))
# baseline_results <- readRDS(file = file.path(peru.province.ensemble.out.dir,
#                  "baseline_results_2010_2021.RDS"))
# baseline_results
...
setnames(baseline_dir.pred_dt_2010_2021,
         c("PRED.V1", "SAMPLE"),
         c("prediction", "sample"))
...
baseline_dir.pred_dt_2010_2018 <- subset(baseline_dir.pred_dt_2010_2021,
                                               YEAR < 2018)
...
setnames(baseline_dir.pred_dt_2010_2018_for_scoring,
...
baseline_dir.pred_dt_2018_2021 <- subset(baseline_dir.pred_dt_2010_2021,
                                               YEAR >= 2018)
...
setnames(baseline_dir.pred_dt_2018_2021_for_scoring,
         c("PROVINCE", "end_of_month"),  c("location", "target_end_date"))
...
setnames(baseline_log_cases.pred_dt_2010_2021,
         c("PRED.V1", "SAMPLE"),
         c("prediction", "sample"))
baseline_log_cases.pred_dt_2010_2021 <- merge(baseline_log_cases.pred_dt_2010_2021, 
                                              subset(ptl_province_inla_df,
                                                     select = c("PROVINCE", "TIME", "MONTH", "YEAR", "CASES",
                                                                "LAT_PROV_IND", "LONG_PROV_IND", "end_of_month")),
                                       by = c("PROVINCE","TIME"))
...
setnames(baseline_log_cases.pred_dt_2010_2018_for_scoring,
         c("PROVINCE", "end_of_month"),  c("location", "target_end_date"))
...
saveRDS(quantile_baseline_log_cases.pred_dt_2010_2018, file = file.path(peru.province.out.dir, 
...
setnames(baseline_log_cases.pred_dt_2018_2021_for_scoring,
         c("PROVINCE", "end_of_month"),  c("location", "target_end_date"))
...
saveRDS(baseline_log_cases.pred_dt_2010_2018, file = file.path(peru.province.inla.data.out.dir, 
                                                        paste0("baseline_log_cases.pred_dt_2010_2018.RDS")))
saveRDS(quantile_baseline_log_cases.pred_dt_2018_2021, file = file.path(peru.province.out.dir, 
