process_predictions <- function(data){
  summary_accuracy_performance_median <-
    data[ , list(r2 = caret::R2(MEDIAN, DIR),
...
process_quantile_predictions <- function(models_dt){
  results_dt <- models_dt[which(quantile == 0.5), 
                          list(r2 = caret::R2(prediction, true_value),
...
quantile_threshold_150_function <- function(data) {
  results_dt <- data.table(true_pos = rep(0, 1),
                           false_pos = rep(0, 1),
  ...
  for(i in 1:length(unique(data$quantile))){
    ...
    confusion_mat <- confusionMatrix(data=as.factor(quantile_data$OUTBREAK_150_PRED), 
    ...
    roc_150 <- roc(quantile_data$OUTBREAK_150_TRUE, 
...
quantile_threshold_50_function <- function(data) {
  ...
  for(i in 1:length(unique(data$quantile))){
    ...
    print(dim(quantile_data))
    confusion_mat <- confusionMatrix(data=as.factor(quantile_data$OUTBREAK_50_PRED), 
    ...
    roc_50 <- roc(quantile_data$OUTBREAK_50_TRUE, 
...
quantile_threshold_150_function_choose_cut_off <- function(data) {
  results_dt <- data.table(cut_off = (unique(data$quantile)),
...
roc_plot_all_models_function <- function(results_dt,
  ...
  roc_plot <- ggplot(roc_dt)+
    geom_line(aes(x = inv_spec, y = sens, color = model_factor), alpha = 0.45,
              linewidth = 0.85)+theme_bw()+scale_x_continuous(labels=scales::percent)+
    scale_y_continuous(labels=scales::percent)+
    scale_x_continuous(labels=scales::percent)+
    geom_abline(intercept = 0, slope = 1,
                linewidth  = 1.2, color = "grey")+ 
    gghighlight(model_factor == "Median *", use_direct_label = FALSE,
  ...
  if(!is.null(onset)){outbreak_threshold <- paste0("onset_", outbreak_threshold)}
  ggsave(roc_plot, filename = file.path(peru.province.ensemble.out.dir,
...
roc_plot_function <- function(results_dt,
  ...
  for(i in 1:length(unique(results_dt$model))){
    ...
    if(outbreak_threshold == 50){
      ...
      confusion_mat <- confusionMatrix(data=as.factor(tmp_dt$OUTBREAK_PRED_50), 
                                       reference = as.factor(tmp_dt$OUTBREAK_TRUE_50),
                                       positive = "1")
      roc_50 <- roc(tmp_dt$OUTBREAK_TRUE_50, 
      ...
      roc_50_object <- ggplot(tmp_roc_dt)+
        geom_line(aes(x = inv_spec, y = sens), color = "mediumpurple4", linewidth = 1.5)+theme_bw()+
        labs(y= "Sensitivity", x = "1 - Specificity (%)")+theme_bw()+
        scale_x_continuous(labels=scales::percent)+
        scale_y_continuous(labels=scales::percent)+ geom_abline(intercept = 0, slope = 1,
                    # linetype = "dashed", 
                      linewidth  = 1.2,
                      color = "grey")+ 
        annotate("text", x = 0.65, y = 0.25, label = paste("AUC = ", c(round(roc_50$auc, 2))),
                 size = 11)+
      ...
      if(!is.null(onset)){new_model_in_q <- paste0(new_model_in_q, "_onset")}

      ggsave(roc_50_object, filename = file.path(peru.province.ensemble.out.dir,
      ...
      confusion_mat <- confusionMatrix(data=as.factor(tmp_dt$OUTBREAK_PRED_150), 
                                       reference = as.factor(tmp_dt$OUTBREAK_TRUE_150),
                                       positive = "1")
      roc_150 <- roc(tmp_dt$OUTBREAK_TRUE_150, 
      ...
      roc_150_object <- ggplot(tmp_roc_dt)+
        ...
        scale_y_continuous(labels=scales::percent)+ geom_abline(intercept = 0, slope = 1,
                                                                ...
                                                                linewidth  = 1.2,
                                                                color = "grey")+ 
        annotate("text", x = 0.65, y = 0.25,label = paste("AUC = ", c(round(roc_150$auc, 2))),
                 size = 11)+
        theme(text = element_text(size = 28),
              axis.text.x = element_text(size=25),
              axis.text.y = element_text(size=25),
      ...
      ggsave(roc_150_object, filename = file.path(peru.province.ensemble.out.dir,
  ...
  for(i in 1:length(unique(results_dt$model))){
    ...
    if(outbreak_threshold == 50){
      ...
      tmp_plot_binary <- 
        ...
        geom_col(aes(fill = INDICATOR), width = 0.7,
                 alpha = 0.7)+
        ...
        scale_fill_discrete(name = "Classification")+theme_bw()+
        ...
        annotate("text", x = 100, y = 330, label = paste("AUC = ", c(tmp_interval_outbreak_50_results$auc)),
                 size = 11)+
        annotate("text", x = 100, y = 275, label = paste("Sensitivity = ", c(tmp_interval_outbreak_50_results$true_pos)),
                 size = 11)+
        annotate("text", x = 100, y = 220, label = paste("Specificity = ", c(tmp_interval_outbreak_50_results$Specificity)),
                 size = 11)+
      ...
      ggsave(tmp_plot_binary, filename = file.path(peru.province.ensemble.out.dir, 
      ...
      tmp_plot_binary <- 
        ...
        # + annotate('text', x = 0, y = 0, 
        #            label = "AUC==0.6 ",parse = TRUE,size=10)+
        annotate("text", x = 100, y = 330, label = paste("AUC = ", c(tmp_interval_outbreak_150_results$auc)),
                 size = 11)+
        annotate("text", x = 100, y = 275, label = paste("Sensitivity = ", c(tmp_interval_outbreak_150_results$true_pos)),
                 size = 11)+
        annotate("text", x = 100, y = 220, label = paste("Specificity = ", c(tmp_interval_outbreak_150_results$Specificity)),
                 size = 11)+
        # geom_label(x=8, y=0.005*10^4, label="First vaccinations",
        #            size=10, fontface = 3, color = "gray27")+
        # geom_label(x=12, y=0.00485*10^4, label="Full vaccination \n becoming \n prevalent",
  ...
  for(i in 1:length(unique(results_dt$model))){
    ...
    if(outbreak_threshold == 50){
      ...
      tmp_plot_binary <- 
        ...
        scale_colour_discrete(name = "Classification")+theme_bw()+
        ...
        # + annotate('text', x = 0, y = 0, 
        #            label = "AUC==0.6 ",parse = TRUE,size=10)+
        annotate("text", x = 300, y = 0.75, label = paste("AUC = ", c(tmp_interval_outbreak_50_results$auc)),
                 size = 11)+
        annotate("text", x = 300, y = 0.5, label = paste("Sensitivity = ", c(tmp_interval_outbreak_50_results$true_pos)),
                 size = 11)+
        annotate("text", x = 300, y = 0.25, label = paste("Specificity = ", c(tmp_interval_outbreak_50_results$Specificity)),
                 size = 11)+
        # geom_label(x=8, y=0.005*10^4, label="First vaccinations",
        #            size=10, fontface = 3, color = "gray27")+
        # geom_label(x=12, y=0.00485*10^4, label="Full vaccination \n becoming \n prevalent",
      ...
      tmp_plot_binary <- 
        ...
        # + annotate('text', x = 0, y = 0, 
        #            label = "AUC==0.6 ",parse = TRUE,size=10)+
        annotate("text", x = 100, y = 300, label = paste("AUC = ", c(tmp_interval_outbreak_150_results$auc)),
                 size = 11)+
        annotate("text", x = 100, y = 300, label = paste("Sensitivity = ", c(tmp_interval_outbreak_150_results$true_pos)),
                 size = 11)+
        annotate("text", x = 100, y = 300, label = paste("Specificity = ", c(tmp_interval_outbreak_150_results$Specificity)),
                 size = 11)+
        # geom_label(x=8, y=0.005*10^4, label="First vaccinations",
        #            size=10, fontface = 3, color = "gray27")+
        # geom_label(x=12, y=0.00485*10^4, label="Full vaccination \n becoming \n prevalent",
...
visualise_and_summarise_quantile_results <- function(model_results,
  ...
  results_dt <- model_results[which(quantile == 0.5),
                              list(r2 = caret::R2(prediction, true_value),
                                   mae = caret::MAE(prediction, true_value)),
                              by = c("model", "location")] #R^2 for plot
  ...
  wis_decomposition_facet_plot_over_time <- 
    ...
    theme(text = element_text(size = 28),
          ...
          legend.position = "bottom", legend.key.size = unit(1.5, 'cm'),
          plot.margin=unit(c(1,1,1,1),"cm"))
  ggsave(wis_decomposition_facet_plot_over_time, filename = file.path(plot_directory,
  ...
  for(i in 1:length(unique(pred_dt$model))){
    ...
    tmp_pred_plot <- ggplot(tmp_pred_dt)+ geom_point(aes(x = target_end_date, y = true_value, col = "Observed"))+ 
      ...
      theme_bw()+ guides(colour = guide_legend(override.aes = list(size=6)))+
      xlab("Time")+ylab(y_text)+labs(title = model_in_q)+
    ...
    model_in_q <- toString(model_in_q)
    new_model_in_q <- gsub(" \\*", "", model_in_q)
    ggsave(tmp_pred_plot, filename = file.path(plot_directory,
    ...
    wis_decomposition_plot_over_time <- 
      ...
      theme(text = element_text(size = 20),
            ...
            legend.text=element_text(size=25),
            legend.position = "bottom", legend.key.size = unit(1.5, 'cm'))
    ggsave(wis_decomposition_plot_over_time, filename = file.path(plot_directory,
  ...
  if(!is.null(outbreak_results)){
    ...
    summary_score_table <- merge(summary_score_table,
                                 outbreak_results, by = "model")
    summary_score_table <- 
      subset(summary_score_table, 
             select = c("model", "interval_score", "bias","coverage",  "true_pos", 
                        "false_pos",  "auc", "ae_median", "r2"))
    summary_score_table <- 
      summary_score_table[order(interval_score, decreasing = FALSE), ]
    setnames(summary_score_table, 
             c("Model", "WIS", "Bias", "PICoverage", 
    ...
    setcolorder(summary_score_table,
                c("Model","WIS", "Bias", "PICoverage", "R2",
  ...
  return(list(list_pred_plots, summary_score_table,
...
interval_coverage_function <- function(data,
  ...
  interval_coverage_single_plot <- 
    ...
    geom_line(aes(y = coverage * 100), linewidth = 1.6,
              alpha = 0.6) +
