...
#Time-GPT
quantile_fine_tuned_timegpt_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
...
quantile_fine_tuned_no_covars_timegpt_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
...
#TCN
quantile_tcn_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
                         "quantile_tcn_dir_preds_dt.RDS"))
quantile_tcn_dir_preds_dt <- subset(quantile_tcn_dir_preds_dt,
                                    select = scoring_columns)

#Climate
climate_2018_2021_forecast_quantile_dt <- readRDS(file = file.path(peru.province.inla.data.out.dir, 
...
sarima_2010_2018_forecast_quantile_dt <- readRDS(file = file.path(peru.province.out.dir, 
                                                                  paste0("sarima_2010_2018_forecast_quantile_dt.RDS")))
sarima_2018_2021_forecast_quantile_dt <- readRDS(file = file.path(peru.province.out.dir, 
...
quantile_sarima_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
...
quantiles_random_forest_dir.pred_2018_2021 <- 
  readRDS(file = file.path(peru.province.xgb.out.dir, 
...
quantile_baseline_dir.pred_dt_2018_2021 <- 
  readRDS(file = file.path(peru.province.out.dir, 
                           paste0("quantile_baseline_dir.pred_dt_2018_2021.RDS")))

quantile_dir_components_dt <- rbind(quantile_fine_tuned_timegpt_dir_preds_dt,
      quantile_fine_tuned_no_covars_timegpt_dir_preds_dt,
...
quantile_dir_components_dt %>% score() %>% summarise_scores(by = c("model"))
...
quantile_dir_components_plus_ensembles_dt_scores <- quantile_dir_components_plus_ensembles_dt %>% score() %>% 
...
quantile_dir_components_plus_ensembles_dt %>% score() %>%
...
iteratively_trained_weights_dir_dt <- iteratively_train_quantile_ensemble(quantiles = quantiles,
...
weighted_quantile_dir_forecasts_2018_2021_dt <- 
  weighted_quantile_dir_dt[, list(prediction = sum(TRAINED_WEIGHT * prediction),
...
tmp <- subset(wis_expanding_dir_results_spatially_homoegenous, 
...
ggplot(wis_expanding_dir_results_space_dependent,
...
#Historical ----
ptl_province_2010_2018_data <- subset(ptl_province_inla_df,
                                      YEAR < 2018)
...
#TimeGPT
quantile_historical_fine_tuned_timegpt_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
...
quantile_historical_fine_tuned_timegpt_dir_preds_dt <- 
  subset(quantile_historical_fine_tuned_timegpt_dir_preds_dt, 
         select = scoring_columns)
quantile_historical_no_covars_fine_tuned_timegpt_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
...
#Climate
climate_2010_2018_forecast_quantile_dt <- readRDS(file = file.path(peru.province.inla.data.out.dir, 
...
#TCN
quantile_historical_tcn_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
...
#SARIMA
quantile_historical_sarima_dir_preds_dt <- readRDS(file = file.path(peru.province.python.out.dir,
...
quantile_baseline_dir.pred_dt_2010_2018 <- readRDS(file = file.path(peru.province.out.dir,
                                                                    "quantile_baseline_dir.pred_dt_2010_2018.RDS"))
quantile_baseline_dir.pred_dt_2010_2018

historical_quantile_dir_components_dt <- rbind(quantile_historical_fine_tuned_timegpt_dir_preds_dt,
                                    quantile_historical_no_covars_fine_tuned_timegpt_dir_preds_dt,
...
historical_quantile_dir_components_dt %>% score() %>% summarise_scores(by = "model")
...
historical_quantile_dir_components_plus_ensembles_dt_scores <- historical_quantile_dir_components_plus_ensembles_dt %>% score() %>% 
  summarise_scores(by = c("model"))
...
input_quantile_dt <- merge(quantile_dir_components_dt,
                           tmp, by = c("model", "location", "target_end_date"))
input_quantile_dt
...
quantile_dir_components_dt
input_quantile_dt <- merge(quantile_dir_components_dt,
                           tmp, by = c("model", "target_end_date"))
input_quantile_dt
input_quantile_dt[, true_value:= round(true_value, digits = 8)]
input_quantile_dt
...
quantile_dir_components_plus_ensembles_dt <- rbind(quantile_dir_components_plus_ensembles_dt,
                                                   trained_ensemble_by_province_quantile_dir_forecasts_2018_2021_dt,
                                                   trained_ensemble_quantile_dir_forecasts_2018_2021_dt)
...
#Outbreak Detection ----
model_nums_dt <- data.table(model = unique(quantile_dir_components_plus_ensembles_dt$model),
...
tmp <- subset(historical_quantile_dir_components_plus_ensembles_dt,
              model == "EW-Mean *")
tmp[, model := "Prov-Trained *"]
tmp2 <- subset(historical_quantile_dir_components_plus_ensembles_dt,
               model == "EW-Mean *")
tmp2[, model := "Trained *"]
tmp
tmp2
tmp <- rbind(historical_quantile_dir_components_plus_ensembles_dt,
             tmp,
             tmp2)
...
for(i in 1:length(all_interval_estimates_2010_2021.split)){
  ...
  all_interval_estimates_2010_2021_with_150_thresholds <- 
    rbind(all_interval_estimates_2010_2021_with_150_thresholds,
          model_df)
  if(i %%2 == 0){gc()}
}
...
all_interval_estimates_2018_2021_with_150_thresholds <-
  subset(all_interval_estimates_2010_2021_with_150_thresholds,
         YEAR >=2018)
...
for(i in 1:length(quantile_dir_components_plus_ensembles_dt.split)){
  tmp <- subset(data.table(all_interval_estimates_2018_2021_with_150_thresholds.split[[i]]),
  ...
  moving_outbreak_150_quantile_2018_2021 <- 
    rbind(moving_outbreak_150_quantile_2018_2021,
          tmp_interval_outbreak_150_results)
}
...
for(i in 1:length(all_interval_estimates_2010_2021.split)){
  ...
  all_interval_estimates_2010_2021_with_50_thresholds <- 
    rbind(all_interval_estimates_2010_2021_with_50_thresholds,
          model_df)
  if(i %%2 == 0){gc()}
}
...
all_interval_estimates_2018_2021_with_50_thresholds <-
  subset(all_interval_estimates_2010_2021_with_50_thresholds,
         YEAR >=2018)
tmp
all_interval_estimates_2018_2021_with_50_thresholds.split <- 
  dlply(all_interval_estimates_2018_2021_with_50_thresholds, .(model))
for(i in 1:length(quantile_dir_components_plus_ensembles_dt.split)){
  tmp <- subset(data.table(all_interval_estimates_2018_2021_with_50_thresholds.split[[i]]),
  ...
  moving_outbreak_50_quantile_2018_2021 <- 
    rbind(moving_outbreak_50_quantile_2018_2021,
          tmp_interval_outbreak_50_results)
}
moving_outbreak_50_quantile_2018_2021
...
historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt <- 
  rbind(historical_quantile_dir_components_plus_ensembles_dt,
             tmp,
             tmp2)
...
historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt <- 
  merge(historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt,
        first_outbreak_months, by = c("location", "YEAR"),
             all.x = TRUE)
historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt[which(is.na(FIRST_OUTBREAK_50_MONTH)),
    FIRST_OUTBREAK_50_MONTH:= 13] #If never occurs in year
#Subset such that only have portion of year on or before first onset month
historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt <- 
...
historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt <- 
  merge(historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt,
        first_outbreak_months, by = c("location", "YEAR"),
        all.x = TRUE)
historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt[which(is.na(FIRST_OUTBREAK_150_MONTH)),
                                                                             FIRST_OUTBREAK_150_MONTH:= 13] #If never occurs in year
#Subset such that only have portion of year on or before first onset month
historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt <- 
...
quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt <- 
  merge(quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt,
        first_outbreak_months, by = c("location", "YEAR"),
        all.x = TRUE)
quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt[which(is.na(FIRST_OUTBREAK_50_MONTH)),
                                                                             FIRST_OUTBREAK_50_MONTH:= 13] #If never occurs in year
#Subset such that only have portion of year on or before first onset month
quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt <- 
...
quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt <- 
  merge(quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt,
        first_outbreak_months, by = c("location", "YEAR"),
        all.x = TRUE)
quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt[which(is.na(FIRST_OUTBREAK_150_MONTH)),
                                                                  FIRST_OUTBREAK_150_MONTH:= 13] #If never occurs in year
#Subset such that only have portion of year on or before first onset month
quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt <- 
...
#Moving threshold 
all_interval_estimates_2010_2021 <- rbind(historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt, 
                                          quantile_dir_components_plus_ensembles_dt_only_season_onset_150_dt)
all_interval_estimates_2010_2021.split <- dlply(all_interval_estimates_2010_2021, .(model))
all_interval_estimates_2010_2021
all_interval_estimates_2010_2021[, length(true_value), by = "model"]
...
for(i in 1:length(all_interval_estimates_2010_2021.split)){
  ...
  all_interval_estimates_2010_2021_with_onset_150_thresholds <- 
    rbind(all_interval_estimates_2010_2021_with_onset_150_thresholds,
          model_df)
  if(i %%2 == 0){gc()}
}
all_interval_estimates_2010_2021_with_onset_150_thresholds
...
all_interval_estimates_2018_2021_with_onset_150_thresholds <- 
  subset(all_interval_estimates_2018_2021_with_onset_150_thresholds,
         YEAR >= 2018)
...
all_interval_estimates_2018_2021_with_onset_150_thresholds
for(i in 1:length(quantile_dir_components_plus_ensembles_dt.split)){
  tmp <- subset(data.table(all_interval_estimates_2018_2021_with_onset_150_thresholds.split[[i]]),
                quantile == OUTBREAK_150_THRESHOLD)
  tmp <- unique(tmp)
  tmp_interval_outbreak_150_results <- 
    quantile_threshold_150_function(tmp)
...
all_interval_estimates_2010_2021 <- rbind(historical_quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt, 
                                          quantile_dir_components_plus_ensembles_dt_only_season_onset_50_dt)
all_interval_estimates_2010_2021.split <- dlply(all_interval_estimates_2010_2021, .(model))
...
for(i in 1:length(all_interval_estimates_2010_2021.split)){
  ...
  all_interval_estimates_2010_2021_with_onset_50_thresholds <- 
    rbind(all_interval_estimates_2010_2021_with_onset_50_thresholds,
          model_df)
  if(i %%2 == 0){gc()}
}
...
quans <- list()
for(i in 1:length(quantile_dir_components_plus_ensembles_dt.split)){
  tmp <- subset(data.table(all_interval_estimates_2010_2021_with_onset_50_thresholds.split[[i]]),
                quantile == OUTBREAK_50_THRESHOLD)
  tmp <- unique(tmp)
  ...
  moving_outbreak_onset_50_quantile_2018_2021 <- 
    rbind(moving_outbreak_onset_50_quantile_2018_2021,
          tmp_interval_outbreak_50_results)
}
quans
tmp_interval_outbreak_50_results
...
new_model_names <- c("Baseline", "Bayes-Climate", "EW-Mean *",
...
historical_new_model_names <- c("Baseline", "Bayes-Climate", "EW-Mean *",
                                "EW-Mean-NoBase *", "Ew-Mean-NoBayes *", "EW-Mean-NoCov *",
                                "TimeGPT", "TimeGPT-NoCov ", "Median *", "Median-NoBase *",
