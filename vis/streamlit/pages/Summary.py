...
def get_filename(model, field='prediction'):
    match metric:
        case 'cases_samples':
            filename = Path(FOLDER_PATH) / model / "pred_log_cases_samples_forecasting.csv"
        ...
        case 'cases_quantiles':
            ...
            data_transform = lambda df: np.exp(df[df['quantile'] == 0.5][field].values[-1]) + 1  # last value (should only be one)
            tooltip_text = "Cases (log): {data}"
        case 'log_cases_quantiles':
            filename = Path(FOLDER_PATH) / model / "pred_log_cases_quantiles_forecasting.csv"
            data_transform = lambda df: df[df['quantile'] == 0.5][field].values[-1]  # last value (should only be one)
            tooltip_text = "Log cases: {data}"
        case _:
